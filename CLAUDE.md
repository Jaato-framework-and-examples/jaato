...
- [Forge Plugins for the Web Coder](docs/design/forge-plugins.md) - Proposed: generalises the per-user GitHub integration so GitLab, Gitea/Forgejo and similar forges plug in without copying it. BFF-only (the daemon's `app://` resolver and #1228 scrub grant are already forge-neutral). A host keeps the grant store, refresh lock, bindings, workspace writes and `secret.resolve` dispatch; a plugin only describes a forge (OAuth endpoints, identity, env var, git username, guidance file). Configured instances (`forges:`, id = `app://` name), a composed `.gitconfig` with per-host credentials and `includeIf` commit identity, a static in-tree registry, and a five-step plan starting with a no-behaviour-change extraction.
