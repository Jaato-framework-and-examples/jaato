...
- [Session Group Messaging](docs/design/session-group-messaging.md) - Assessment and design for any-to-any messaging between sessions that share a group (`created_by` owner or `cascade_driver_id`), waking an idle, detached, or unloaded target to process the message, with a payload of text, file references and text/binary attachments. Inventories the primitives that already exist (`send_to_sibling`, `session.send`, `session.wake`, `deliver_prompt_to_session` / `offer_message`, the workspace index, file staging) against the requirement, names the eight missing blocks (a group predicate, cross-workspace resolution, wake-on-delivery for a peer, a durable inbox, an envelope with file references, cross-workspace file copy, the model/SDK surface, observability) and proposes a three-phase rollout that composes rather than duplicates, landing the peer tools (the new `send_to_session` / `list_group_sessions` and the relocated `send_to_sibling` / `list_siblings`) in a new `courier` plugin.
