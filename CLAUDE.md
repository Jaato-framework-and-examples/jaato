...
- **plugins/**: Plugin system with three plugin types:
  ...
  - **Session Plugins**: Session persistence for save/resume functionality
    - `session/base.py`: `SessionPlugin` protocol, `SessionState`, `SessionConfig`
    - `session/file_session.py`: File-based persistence implementation
    - Integrates with `SessionManager` for multi-session disk persistence
