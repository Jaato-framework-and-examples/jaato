...
1. **Search Index**: A pre-built JSON index (`docs/web/assets/js/search-index.json`) contains all page content, titles, headings, and snippets.
...
3. **Search UI**: Custom JavaScript in `docs/web/assets/js/docs.js` handles:
   - Loading the index
   - Building the Lunr index
   - Performing searches
   - Displaying results with dropdown

4. **Styling**: CSS in `docs/web/assets/css/style.css` provides the search results dropdown appearance.
...
This will:
1. Scan all HTML files in `docs/web/`
2. Extract titles, headings, and text content
3. Generate a new `docs/web/assets/js/search-index.json` file
...
============================================================
Scanning /home/user/jaato/docs/web for HTML files...
Found 22 HTML files
...
Writing index to: /home/user/jaato/docs/web/assets/js/search-index.json
...
1. Download Lunr.js:
   ```bash
   curl -o docs/web/assets/js/lunr.min.js https://unpkg.com/lunr@2.3.9/lunr.min.js
   ```
...
Edit `docs/web/assets/js/docs.js`, function `performSearch()`:
...
Edit `docs/web/assets/css/style.css`, `.search-results` section:
