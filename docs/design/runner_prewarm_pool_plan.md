...
- `PoolManager.start_replenishment()` spawns a daemon thread that watches `idle_count() < target_size` and asks the template for a fork-slot to refill. Daemon's `start()` invokes it after `spawn_initial_slots()`.  (Amended #898: the watched quantity is `unreserved_idle_count()`, and a `max_size` ceiling bounds the total — see the decision log entry.)
...
- **2026-09-08 (#898)** — Capacity accounting split in two.  `target_size` had been the floor on TOTAL idle slots while slots were allocated PER-TENANT (cascade affinity, cross-cascade reuse forbidden by design), so a pool at capacity could be empty from the point of view of every tenant but one: two idle slots affined to cascade B, `acquire_slot(cascade=A)` returning `None`, and replenishment reading `idle_count() == 2 >= 2` and never forking.  Reported live as a `session.new` that got no daemon attention for its whole 60 s budget while the other tenant ran.  Fix: `target_size` becomes the floor on the **unreserved** subset (`unreserved_idle_count()`), reservations sit on top of it, and a new `max_size` (`JAATO_RUNNER_POOL_MAX_SIZE`, default `2 x target_size`) is the ceiling on the total — the memory bound on the growth that per-tenant reservations imply, at 129–187 MB a slot.  Eviction at the ceiling reordered on the same principle, liveness over warmth: a returner whose cascade is mid-run displaces the **stalest reservation** first, and a pure-idle resident only when there is no reservation to spend.  A cascade that loses its reservation still runs, cold.  Deliberately NOT taken: the "capacity floor per active cascade" shape, which needs the pool to know which cascades are live — and the reporting run *crashed*, so a tenant that dies without finalizing would pin a slot indefinitely.  Nothing here depends on a tenant declaring itself finished; the 300 s cascade-idle sweep stays the backstop.
