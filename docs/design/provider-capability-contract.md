...
| `reasoning_replay` | An assistant turn's reasoning goes BACK to the model on the next request: the provider sets `replay_reasoning = True` (so the streaming/batch paths emit a leading `Part.thought` and the session keeps it in history) and its converter maps that part to wire fields (`reasoning_content` by default, via `_reasoning_replay_fields`). Required by the thinking models of MiMo (400 without it), Kimi and MiniMax; see [MiniMax, Kimi and MiMo providers](minimax-kimi-mimo-providers.md) §3. | Behavioral, both directions: a declared provider's converter must put a thought part's text on the assistant message with `content: ""` next to `tool_calls`; an undeclared provider must not set `replay_reasoning`. |
