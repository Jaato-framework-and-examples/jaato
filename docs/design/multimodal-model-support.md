...
| Output (model→media) | ✅ model-emitted audio streams as `MediaDelta` and reaches clients as `CLIENT`-audience chunks (#824); declared by `output_media` on `azure_openai`, `doubleword`, `nebius`, `nim`, `openai`, `openrouter`, `ovhcloud` and `zhipuai_openai` — the native `openai` provider decodes it on both of its wires, the Responses events being the same split (bytes, then words) under different names |
