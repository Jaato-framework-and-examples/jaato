...
| Output (model→media) | ✅ model-emitted audio streams as `MediaDelta` and reaches clients as `CLIENT`-audience chunks (#824); declared by `output_media` on `doubleword`, `kimi`, `mimo`, `nebius`, `nim`, `openrouter`, `ovhcloud` and `zhipuai_openai` |
