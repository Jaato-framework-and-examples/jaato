...
|     **§7c step 3b** | Remaining INTERNAL bucket — refactor `set_apparmor_confinement` (583), `set_runtime_limits` (668), `set_reference_authorizer` (694), `_build_tool_id_mappings` (1290), `send_message` / `_start_model_thread` `get_session()` reads.  Most of these mutate session-internal executor state that becomes runner-tier post-seat-flip; some can be deleted outright. | Pending. |
|     **§7c step 3c** | WIRING bucket — `set_session_plugin` (2251), `configure_plugins_only` (1765), `configure_tools` (1784, 4113), `set_gc_plugin` (1854, 4122).  Each requires a runner-side equivalent + a daemon-side wrapper that forwards.  Largest scope sub-step. | Pending. |
