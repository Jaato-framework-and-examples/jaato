...
| **§7c step 6** | Atomic seat-flip — absorbs the read migrations folded from step 5.  Decomposed into 3 reviewable commits per the §7b.3-withdrawal note's "the §7c change-set may land as 2-3 reviewable commits": **6a** `_jaato`-field removal + ~15 `if self._jaato:` truthiness collapses to `if self._runner_rpc:` (mechanical, large diff); **6b** INTERNAL `get_session()` refactors (~8 sites; architectural); **6c** WIRING deletions + write-both daemon-leg drops + response-handler rewiring (the §7b.3 fold).  Removes the `JAATO_RUNNER_HOSTS_SESSION` env-var doc references along the way. | Pending. |
