...
|     **§7c step 6.1** | Add 3 new runner-RPC handlers (each §7b.2-scale: handler + daemon-side wrapper + unit + e2e tests): `session.set_reference_authorizer`, `session.snapshot_instruction_budget`, `session.inject_prompt`.  Prerequisites for the daemon-side reader migrations in 6.2. | **1 of 3 shipped:** `session.set_reference_authorizer` (12 tests).  Wire shape: bool flag (`enabled: bool`) — the Python `ReferenceAuthorizer` object can't cross RPC since it holds a daemon-side `AppArmorManager` reference.  Runner-side references plugin (post-migration) will read the flag via `JaatoSession.is_reference_authorization_enabled()` and use the existing `apparmor.add_reference_fragment` runner→daemon RPC (§3.2.2) to authorize paths.  Remaining: `session.snapshot_instruction_budget`, `session.inject_prompt`. |
