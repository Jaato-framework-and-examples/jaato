...
def completion() -> Rendered:
    ...
    lines = [
        ...
        "  enter_tier has exactly one real attempt left.  Per SESSION, not",
        "  per turn; positive integer (0 is refused — the give-up test is",
        "  `fired >= max`, so a budget of 0 would report NudgeExhausted on",
        "  sessions that completed cleanly).  Inherits like max_turns:",
        "  child overrides, else the minimum across parents.",
        "",
