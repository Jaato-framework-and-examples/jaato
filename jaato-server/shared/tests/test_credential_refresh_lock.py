...
REVERSIONS = [
    Reversion(
        target="jaato-server/shared/credential_lock.py",
        find="""        state = load()
        if state is None:
            return None
...
_WORKER = textwrap.dedent(
    ...
    from shared.credential_lock import refresh_under_lock
...
def _server_dir() -> str:
    import shared
    return str(Path(shared.__file__).resolve().parent.parent)
...
def test_margin_is_configurable(monkeypatch) -> None:
    from shared.credential_lock import REFRESH_MARGIN_ENV, refresh_margin_seconds
...
@pytest.mark.parametrize("module_path", [
    "shared.plugins.model_provider.anthropic.oauth",
    "shared.plugins.model_provider.antigravity.oauth",
    "shared.plugins.model_provider.github_models.oauth",
])
def test_every_rotating_provider_reaches_the_shared_mechanism(module_path: str) -> None:
    ...
    imported = {
        ...
        if isinstance(node, ast.ImportFrom) and node.module == "shared.credential_lock"
