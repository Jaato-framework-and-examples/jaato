...
# user fills in OLLAMA_HOST) is only created when it does not
...
#     a variant.
#   - Deployment knobs (host) live in the workspace .env, referenced
#     from the profile as ${OLLAMA_HOST}.  Edit the workspace .env
#     after bootstrap.
#
...
# smoke/ → ollama/ → model_provider/ → plugins/ → shared/ → jaato-server/ → repo root
JAATO_REPO="$(cd "$SMOKE_DIR/../../../../../.." && pwd)"

# Defaults
WORKSPACE="/tmp/jaato-ollama-smoke"
...
After bootstrap, edit <workspace>/.env to set OLLAMA_HOST.
...
if [[ -f "$WORKSPACE/.env" ]]; then
    echo "==> Preserving existing $WORKSPACE/.env (edit it if OLLAMA_HOST needs to change)"
else
    cp -f "$SMOKE_DIR/.env.example" "$WORKSPACE/.env"
    echo "==> Created $WORKSPACE/.env from .env.example — edit OLLAMA_HOST to point at your endpoint"
fi
...
Bootstrap complete.  Next:
  ...
  1. Edit  $WORKSPACE/.env  to set OLLAMA_HOST.
  2. Run the smoke:
       cd $WORKSPACE
