...
# user fills in VLLM_HOST) is only created when it does not
...
#     from the profile as ${VLLM_HOST}.  Edit the workspace
#     .env after bootstrap.
#
...
# smoke/ → vllm/ → model_provider/ → plugins/ → shared/ → jaato-server/ → repo root
JAATO_REPO="$(cd "$SMOKE_DIR/../../../../../.." && pwd)"

# Defaults
WORKSPACE="/tmp/jaato-vllm-smoke"
...
if [[ -f "$WORKSPACE/.env" ]]; then
    echo "==> Preserving existing $WORKSPACE/.env (edit it if VLLM_HOST needs to change)"
else
    cp -f "$SMOKE_DIR/.env.example" "$WORKSPACE/.env"
    echo "==> Created $WORKSPACE/.env from .env.example — edit VLLM_HOST to point at your endpoint"
fi
