...
# user fills in LMSTUDIO_HOST) is only created when it does not
...
#     a variant.
#   - Deployment knobs (host, optional api_token) live in the
#     workspace .env, referenced from the profile as ${LMSTUDIO_HOST}.
#     Edit the workspace .env after bootstrap.
#
...
# smoke/ → lmstudio/ → model_provider/ → plugins/ → shared/ → jaato-server/ → repo root
JAATO_REPO="$(cd "$SMOKE_DIR/../../../../../.." && pwd)"

# Defaults
WORKSPACE="/tmp/jaato-lmstudio-smoke"
...
After bootstrap, edit <workspace>/.env to set LMSTUDIO_HOST (and
optionally LMSTUDIO_API_TOKEN).
...
if [[ -f "$WORKSPACE/.env" ]]; then
    echo "==> Preserving existing $WORKSPACE/.env (edit it if LMSTUDIO_HOST needs to change)"
else
    cp -f "$SMOKE_DIR/.env.example" "$WORKSPACE/.env"
    echo "==> Created $WORKSPACE/.env from .env.example — edit LMSTUDIO_HOST to point at your endpoint"
fi
...
Bootstrap complete.  Next:
  ...
  1. Edit  $WORKSPACE/.env  to set LMSTUDIO_HOST
     (and optionally LMSTUDIO_API_TOKEN if the server requires it).
  2. Make sure the model named in the profile is loaded in LM Studio
     (LM Studio UI → Chat tab, or:  lms load <model>).
  3. Run any of the three smokes:
       cd $WORKSPACE
