"""Chat smoke for the nim provider — pure text round-trip.
...
NIM endpoint (NVIDIA's hosted API or a self-hosted container) and
round-trip a chat completion.  Tool-calling fidelity is exercised
by the sibling ``smoke_signal_completion.py`` (lifecycle tool only)
...
Run:
    ...
    .venv/bin/python smoke_chat.py --workspace /tmp/jaato-nim-smoke
    # or from inside the workspace:
    cd /tmp/jaato-nim-smoke && .venv/bin/python /path/to/smoke_chat.py
"""
