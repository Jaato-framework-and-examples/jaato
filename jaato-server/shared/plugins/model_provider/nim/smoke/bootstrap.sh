...
# user fills in JAATO_NIM_API_KEY) is only created when it does not
...
#     a variant.
#   - Deployment knobs (api_key, base_url) live in the workspace .env.
#     The profile references the API key as ${JAATO_NIM_API_KEY};
#     base_url is picked up directly by the provider from the
#     environment (resolve_base_url() in nim/env.py).
#
...
# smoke/ → nim/ → model_provider/ → plugins/ → shared/ → jaato-server/ → repo root
JAATO_REPO="$(cd "$SMOKE_DIR/../../../../../.." && pwd)"

# Defaults
WORKSPACE="/tmp/jaato-nim-smoke"
...
After bootstrap, edit <workspace>/.env to set JAATO_NIM_API_KEY
(and JAATO_NIM_BASE_URL for self-hosted endpoints).
...
if [[ -f "$WORKSPACE/.env" ]]; then
    echo "==> Preserving existing $WORKSPACE/.env (edit it if JAATO_NIM_API_KEY needs to change)"
else
    cp -f "$SMOKE_DIR/.env.example" "$WORKSPACE/.env"
    echo "==> Created $WORKSPACE/.env from .env.example — edit JAATO_NIM_API_KEY to your nvapi-... key"
fi
...
Bootstrap complete.  Next:
  ...
  1. Edit  $WORKSPACE/.env  to set JAATO_NIM_API_KEY.
     (Optionally set JAATO_NIM_BASE_URL for a self-hosted endpoint.)
  2. Run the smoke:
       cd $WORKSPACE
