"""Tool-calling smoke for the openrouter provider.

Like ``smoke.py`` but uses the ``openrouter-tools`` profile, which
...
tool schemas in the OpenAI shape the upstream model expects, parses
tool-call arguments correctly, and round-trips the tool result back into
the conversation.  If chat-only ``smoke.py`` is green but this fails,
...
PROFILE = "openrouter-tools"
AGENT = "openrouter-tools"
