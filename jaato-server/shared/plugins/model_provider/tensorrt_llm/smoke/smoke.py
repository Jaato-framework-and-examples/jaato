"""Minimal SDK harness for the tensorrt_llm provider.

Connects to a running jaato daemon over IPC, spawns a session against the
``tensorrt-llm-smoke`` profile + agent, sends one user message, prints
model output as it arrives, and exits on ``TURN_COMPLETED``.

This is a **wire test**: it validates the daemon can reach the remote
``trtllm-serve`` (or Triton OpenAI frontend) and round-trip a chat
completion.  No tools are exercised — function-calling is a separate
smoke (copy this harness, swap the profile to one with ``plugins: ["cli"]``).
...
Run:
    ...
    .venv/bin/python -m smoke --workspace /tmp/jaato-tensorrt-smoke
    # or from inside the workspace:
    cd /tmp/jaato-tensorrt-smoke && .venv/bin/python /path/to/smoke.py
"""
...
PROFILE = "tensorrt-llm-smoke"
AGENT = "tensorrt-llm-smoke"
