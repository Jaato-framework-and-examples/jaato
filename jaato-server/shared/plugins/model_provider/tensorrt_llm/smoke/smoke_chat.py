"""Chat smoke for the tensorrt_llm provider — pure text round-trip.
...
remote ``trtllm-serve`` (or Triton OpenAI frontend) and round-trip
a chat completion.  Tool-calling fidelity is exercised by the
...
Run:
    ...
    .venv/bin/python smoke_chat.py --workspace /tmp/jaato-tensorrt-smoke
    # or from inside the workspace:
    cd /tmp/jaato-tensorrt-smoke && .venv/bin/python /path/to/smoke_chat.py
"""
...
PROFILE = "tensorrt-llm-chat"
AGENT = "tensorrt-llm-chat"
