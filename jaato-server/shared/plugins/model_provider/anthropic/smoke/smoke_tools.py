...
Like ``smoke.py`` but uses the ``anthropic-tools`` profile, which
...
tool schemas in the Anthropic tool-use shape the API expects, parses
tool-call arguments correctly, and round-trips the tool result back
into the conversation.  If chat-only ``smoke.py`` is green but this
fails, the bug is in the tool-call path (schema serialization,
argument parsing, or the model's own tool-calling fidelity), not the
wire.
