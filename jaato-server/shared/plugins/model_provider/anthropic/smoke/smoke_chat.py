"""Chat smoke for the anthropic provider — pure text round-trip.
...
This is the **wire test**: it validates the daemon can reach
``api.anthropic.com`` (or the configured base URL) and round-trip a
Messages API call.  Tool-calling fidelity is exercised by the sibling
...
Run:
    ...
    .venv/bin/python smoke_chat.py --workspace /tmp/jaato-anthropic-smoke
    # or from inside the workspace:
    cd /tmp/jaato-anthropic-smoke && .venv/bin/python /path/to/smoke_chat.py
"""
