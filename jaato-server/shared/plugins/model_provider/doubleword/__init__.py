...
(queued work guaranteed to start within ~1 minute).  The provider
bootstraps the active model's context window and input modalities from
the ``GET /v1/models`` catalog at connect time, with manual override
knobs when the catalog doesn't report them.
