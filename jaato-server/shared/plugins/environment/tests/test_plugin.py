...
class TestAgenticServersReliability:
    """Tests for reliability data in the agentic_servers aspect (Phase 4)."""

    def test_reliability_section_present_for_self(self):
        """When server_reliability is provided, self includes reliability."""
        plugin = EnvironmentPlugin()
        ...
        class MockReliabilityTracker:
            ...
            def get_affinity_score(self, name):
                return 0.95
            def get_state(self, name):
                return None

        plugin.set_gossip_context(
            MockPeerRegistry(), MockHealthCollector(),
            server_reliability=MockReliabilityTracker(),
        )
        result = json.loads(plugin._get_environment({"aspect": "jaato_agentic_servers"}))

        assert "reliability" in result["self"]
        rel = result["self"]["reliability"]
        assert rel["trust_state"] == "trusted"
        assert rel["success_rate_1h"] == 0.97
        assert rel["escalated_tools"] == 0
        assert rel["affinity_score"] is None  # Not applicable for self

    def test_reliability_section_present_for_peers(self):
        """When server_reliability is provided, peers include reliability."""
        plugin = EnvironmentPlugin()
        ...
        class MockPeerEntry:
            config = MockConfig()
            state = type('PeerState', (), {'value': 'healthy'})()
            last_heartbeat = MockHeartbeat()
            last_heartbeat_at = time.monotonic() - 2.0

        class MockPeerRegistry:
            def get_peer_snapshots(self):
                return [MockPeerEntry()]

        class MockState:
            escalation_reason = "DELEGATION_FAILED: 3 consecutive"

        class MockReliabilityTracker:
            def get_self_report(self):
                return {"trust_state": "trusted", "success_rate_1h": 1.0, "escalated_tools": 0}
            def get_affinity_score(self, name):
                return 0.72
            def get_state(self, name):
                return MockState()
        ...
        peer = result["peers"][0]
        assert "reliability" in peer
        rel = peer["reliability"]
        assert rel["trust_state"] == "escalated"
        assert rel["success_rate_1h"] == 0.75
        assert rel["escalated_tools"] == 2
        assert rel["affinity_score"] == 0.72
        assert rel["escalation_reason"] == "DELEGATION_FAILED: 3 consecutive"

    def test_no_reliability_section_without_tracker(self):
        """Without server_reliability, no reliability section appears."""
        plugin = EnvironmentPlugin()
        ...
        assert "reliability" not in result["self"]

    def test_escalation_reason_absent_when_not_escalated(self):
        """escalation_reason should not appear when there's no escalation."""
        plugin = EnvironmentPlugin()
        ...
        class MockPeerEntry:
            ...
            state = type('PeerState', (), {'value': 'healthy'})()
            last_heartbeat = MockHeartbeat()
            last_heartbeat_at = time.monotonic() - 1.0
        ...
        peer = result["peers"][0]
        assert "reliability" in peer
        assert "escalation_reason" not in peer["reliability"]
