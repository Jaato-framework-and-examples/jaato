...
Usage:
    ...
    python -m jaato_server --ipc-socket /tmp/jaato.sock

    # Start with WebSocket only (remote)
    python -m jaato_server --web-socket :8080

    # Start with both
    python -m jaato_server --ipc-socket /tmp/jaato.sock --web-socket :8080
...
from jaato_server.server.session_logging import configure_session_logging
...
class JaatoDaemon:
    ...
    def __init__(
        ...
        # session-mode).
        from jaato_server.server.runner_template import TemplateManager
    ...
    async def start(self) -> None:
        ...
        if self.ipc_socket:
            from jaato_server.server.ipc import JaatoIPCServer, _get_display_path
        ...
        if self.web_socket:
            ...
            from jaato_server.server.websocket import load_tls_context
        ...
        try:
            import pathlib
            from jaato_server.server.wake_ingress import WakeIngressConfig, WakeIngressServer
    ...
    def _load_extensions(self) -> None:
        ...
        **Minimal extension skeleton**::
        ...
        from jaato_server.shared.plugins.gc import discover_gc_plugins
    ...
    def _discover_daemon_plugins(self) -> None:
        ...
        for finder, name, ispkg in pkgutil.iter_modules([str(plugins_dir)]):
            if name.startswith('_') or name in ('base', 'registry'):
                continue
            try:
                module = importlib.import_module(f"jaato_server.shared.plugins.{name}")
...
from jaato_server.server.runner_spawn import (  # noqa: E402, F401 — back-compat re-export
...
def daemonize(log_file: str = DEFAULT_LOG_FILE) -> None:
    ...
    On Windows, re-execs as ``python -m jaato_server`` so the daemon starts
    correctly regardless of whether the caller used ``python -m jaato_server``
    ...
    if sys.platform == "win32":
        ...
        import subprocess
        # Always re-exec via ``-m jaato_server`` so it works whether the caller
        # invoked ``python -m jaato_server --daemon`` or ``jaato-server --daemon``.
        # The console-script .exe is not a Python file, so we cannot just
        # re-use sys.argv[0].
        args = [sys.executable, "-m", "jaato_server"] + sys.argv[1:]
...
def _pid_on_socket(socket_path: Optional[str]) -> Optional[int]:
    ...
    ``-m jaato_server`` / ``jaato-server``) before returning it, so a process that
        ...
        for conn in psutil.net_connections(kind="unix"):
            if conn.laddr == socket_path and conn.pid:
                try:
                    ...
                    continue
                if "-m jaato_server" in cmdline or "jaato-server" in cmdline:
                    return conn.pid
    except Exception:
        return None
    return None
...
def _resolve_ws_app_credentials(args) -> 'Optional[AppCredentialStore]':
    ...
    * the flag with ``--ws-unsafe-no-auth`` -- the point of app credentials is
    ...
    if args.ws_unsafe_no_auth:
        ...
        sys.exit(2)
    from jaato_server.server.ws_tickets import AppCredentialsError, load_app_credentials
    try:
  ...
  python -m jaato_server --restart
    ...
    if args.status:
        ...
        if pid:
            ...
            from jaato_server.server.env_report import format_overrides, read_proc_environ
    ...
    if pid:
        ...
        print(f"  Use 'python -m jaato_server --stop' to stop it")
        sys.exit(1)
    ...
    if args.daemon and not os.environ.get("JAATO_DAEMONIZED"):  # env: internal — set by the daemonizer for its re-exec'd child; not an operator knob
        ...
        from jaato_server.server.env_report import format_overrides
