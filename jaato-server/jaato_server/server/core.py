...
from jaato_server.shared import (
    JaatoRuntime,
    TokenLedger,
    PluginRegistry,
    PermissionPlugin,
...
def _transport_error_details(exc: BaseException) -> Optional[Dict[str, Any]]:
    ...
    """
    from jaato_server.server.runner_rpc_client import RunnerDispatchLost
...
def _deserialize_wire_history(history: Any) -> List[Any]:
    ...
    if not all(isinstance(m, dict) for m in history):
        return list(history)
    try:
        from jaato_server.shared.plugins.session.serializer import deserialize_history
...
def _note_incident(
    ...
    *,
    ...
    """
    from jaato_server.shared.incidents import raise_incident
...
def _record_keeping_env(profile: Any) -> Dict[str, str]:
    ...
    if keeping is None or not getattr(keeping, "chains", False):
        return {}
    from jaato_server.shared.token_accounting import LEDGER_INTEGRITY_ENV
...
class JaatoServer:
    ...
    def __init__(
        ...
        from jaato_server.shared.pricing import PricingTable
    ...
    def disclosure_decision(self) -> "tuple[Optional[str], Optional[str]]":
        ...
        """
        from jaato_server.shared.ai_disclosure import announcement_for
    ...
    def record_disclosure_announcement(
        ...
        **A row with no file to land in is announced at WARNING.**  With
        ...
        """
        from jaato_server.shared.ai_disclosure import announcement_record
    ...
    def _resolve_session_env(self) -> None:
        ...
        if self._workspace_path:
            from jaato_server.shared.plugins.subagent.config import _compute_jdtls_state_root
    ...
    def _with_session_env(self):
        ...
        """
        from jaato_server.shared.session_context import set_session_env, clear_session_env
    ...
    def _build_usage(
        ...
        if not self._pricing_loaded:
            from jaato_server.shared.pricing import load_pricing
            ...
            with self._with_session_env(), self._in_workspace():
                ...
                with _s5.sub("gc_config"):
                    ...
                    if self._profile and self._profile.gc:
                        from jaato_server.shared.plugins.subagent.config import gc_profile_to_plugin_config
            ...
            def on_tool_call_start(self, agent_id, tool_name, tool_args, call_id=None):
                ...
                if tool_args and isinstance(tool_args, dict):
                    ...
                    if (
                        ...
                        from jaato_server.shared.tool_id_map import id_to_name as _id_to_name
    ...
    def _emit_error_termination(
        ...
        *,
        ...
        from jaato_server.shared.incidents import KIND_NUDGE_EXHAUSTED, KIND_SESSION_ERROR
    ...
    def _start_model_thread(
        ...
        from jaato_server.server.session_logging import (
        ...
        def model_thread():
            ...
            try:
                ...
                from jaato_server.server.runner_rpc_client import RunnerRPCTimeout
    ...
    def set_runner_rpc(
        ...
        if rpc_client is not None:
            ...
            from jaato_server.server.runner_rpc_handlers.daemon_plugin_execute import (
