...
.venv/bin/python -m jaato_server --restart
...
The headless client auto-starts the server by default. If you used `--no-auto-start`, make sure the daemon is running (`python -m jaato_server --status`).
