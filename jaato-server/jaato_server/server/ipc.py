...
from jaato_server.shared.peer_identity import PeerCredentials, peer_credentials
from jaato_server.shared.plugins.path_safety import ensure_private_dir
...
def _resolve_event_queue_max() -> int:
    ...
    """
    raw = os.environ.get("JAATO_IPC_EVENT_QUEUE_MAX", "").strip()  # env: per-client IPC event-queue bound; past it the oldest lossy chunks drop (default 2048)
    if raw:
        try:
            parsed = int(raw)
            if parsed > 0:
...
class JaatoIPCServer:
    ...
    def _register_client_tools_ipc(self, client_id, tools, categories=None) -> None:
        ...
        if (session is None or getattr(session, "server", None) is None
            ...
            return
        from jaato_server.server.client_tools import register_client_tools
