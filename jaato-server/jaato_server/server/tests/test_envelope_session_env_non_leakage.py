...
def test_session_record_does_not_serialize_session_env() -> None:
    ...
    envelope content."""
    import jaato_server.server.session_manager as sm
...
def test_waypoint_serialization_drops_session_env() -> None:
    ...
    try:
        from jaato_server.shared.plugins.waypoint import manager as wp_manager
    except ImportError:
...
def test_bootstrap_session_logs_count_not_values(caplog) -> None:
    ...
    import logging
    from jaato_server.server.runner.session import _apply_envelope_session_env

    env = _envelope_with_secret()
    caplog.set_level(logging.INFO, logger="jaato_server.server.runner.session")
