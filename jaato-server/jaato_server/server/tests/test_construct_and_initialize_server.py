...
def _patch_jaato_server():
    ...
    with patch("jaato_server.server.session_manager.JaatoServer", _FakeJaatoServer):
        yield
...
def test_load_session_impl_uses_sub_helper_with_no_client_id(
    ...
    with patch(
        "jaato_server.server.session_manager.SessionManager._construct_and_initialize_server",
        _stub,
    ):
