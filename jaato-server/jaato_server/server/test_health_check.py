...
from jaato_server.server.health_check import (
...
def test_returns_none_when_limit_unreadable(monkeypatch):
    ...
    monkeypatch.setattr(
        "jaato_server.server.health_check._read_inotify_limit", lambda: None,
    )
...
def test_returns_none_below_threshold(monkeypatch):
    ...
    monkeypatch.setattr(
        "jaato_server.server.health_check._read_inotify_limit", lambda: 100,
    )
    monkeypatch.setattr(
        "jaato_server.server.health_check._count_inotify_instances", lambda: 50,
    )
    assert check_inotify_pressure() is None
...
def test_returns_none_at_exactly_80_pct(monkeypatch):
    ...
    monkeypatch.setattr(
        "jaato_server.server.health_check._count_inotify_instances", lambda: 80,
    )
    assert check_inotify_pressure() is None
...
def test_returns_warning_above_threshold(monkeypatch):
    ...
    monkeypatch.setattr(
        "jaato_server.server.health_check._count_inotify_instances", lambda: 85,
    )
...
def test_returns_warning_near_full(monkeypatch):
    ...
    monkeypatch.setattr(
        "jaato_server.server.health_check._count_inotify_instances", lambda: 99,
    )
    msg = check_inotify_pressure()
    assert msg is not None
...
def test_zero_or_negative_limit_treated_as_disabled(monkeypatch):
    ...
    for bad_limit in (0, -1):
        monkeypatch.setattr(
            "jaato_server.server.health_check._read_inotify_limit", lambda: bad_limit,
        )
        monkeypatch.setattr(
            "jaato_server.server.health_check._count_inotify_instances", lambda: 50,
        )
