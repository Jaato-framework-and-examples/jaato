...
def _runtime_limits_report() -> Dict[str, Any]:
    ...
    """
    from jaato_server.shared import runtime_limits as rl
...
def tiers() -> Rendered:
    ...
    """
    from jaato_server.shared import model_tiers as mt
...
def providers() -> Rendered:
    P = introspect.providers()
    from jaato_server.shared.plugins.model_provider.base import CAPABILITY_FIELDS
    rows = []
    data = {}
...
def _scope_summary(EV) -> List[str]:
    ...
    number get smaller.
    """
    from jaato_server.shared.env_scope import AWAITING_TYPED_KEY, SESSION
...
def _env_selected(v, filter_: str, want_untyped: bool) -> bool:
    ...
    """
    from jaato_server.shared.env_scope import SESSION
...
def services(workspace: str = ".") -> Rendered:
    ...
    """
    from jaato_server.shared.plugins.service_connector.schema_store import (
...
def paths() -> Rendered:
    ...
    lines = [
        ...
        "    -> In git, only the AUTHORED half of .jaato/ belongs: `jaato-scaffold",
        "       new gitignore --workspace DIR` writes the .gitignore block that",
        "       keeps profiles/agents/schemas/scripts committable and sessions/,",
        "       logs/, memories/ and <provider>_auth.json out; `validate` checks it.",
        "",
