...
class TestSetSessionAutoWiring(unittest.TestCase):
    ...
    def setUp(self):
        ...
        from jaato_server.shared.session_context import _current_session
...
class TestMemoryPluginFrameworkWiring(unittest.TestCase):
    ...
    def test_jaato_session_calls_plugin_set_session(self):
        ...
        repo_root = here.parents[6]  # tests/ → memory/ → plugins/ → shared/ → jaato_server/ → jaato-server/ → repo
        js = repo_root / "jaato-server" / "jaato_server" / "shared" / "jaato_session.py"
