...
```python
from jaato_server.shared.plugins.subagent import SubagentPlugin
...
```python
from jaato_server.shared.plugins.subagent import SubagentProfile
...
```python
from jaato_server.shared import JaatoRuntime, ProviderConfig
...
| `model_tiers` | object | `{}` | Per-turn tier switching (planner / dispatcher / executor) — see `jaato_server/shared/model_tiers.py` |
...
the named script via `jaato_server/shared/script_loader.py` (resolution chain:
...
`jaato_server/shared/dynamic_instructions.py`) with handles to:
...
`DynamicInstructionsError` in `jaato_server/shared/dynamic_instructions.py`), which is
