...
2. **Directory Scanning** (fallback): Plugins in `jaato_server/shared/plugins/` are auto-discovered for development.
...
For quick development, add a Python file to `jaato_server/shared/plugins/` that implements the `ToolPlugin` protocol.
...
from google.genai import types
from jaato_server.shared.plugins.base import UserCommand
...
4. **Tool Traits**: Tools that write or modify files must declare the `file_writer` trait so the session routes their results through the enrichment pipeline (LSP diagnostics, artifact tracking). Import the constant and add it to the schema:
   ...
   See `TRAIT_FILE_WRITER` in `jaato_server/shared/plugins/model_provider/types.py` for the full contract. Additional traits may be defined in the future using the same pattern.
...
```python
from jaato_server.shared.plugins.base import UserCommand, CommandCompletion
...
**Example:**
```python
from jaato_server.shared.plugins.thinking import create_plugin
...
from jaato_server.shared.plugins.base import ToolPlugin
...
```python
from jaato_server.shared.plugins.session import create_plugin as create_session_plugin
...
```python
from jaato_server.shared.plugins.base import PromptEnrichmentResult
