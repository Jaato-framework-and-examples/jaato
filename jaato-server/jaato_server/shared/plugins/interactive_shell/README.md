...
When running under MSYS2 (detected via `jaato_server.shared.path_utils.is_msys2_environment()`), the plugin tries backends in priority order:
...
|-------|--------|-----------------------------|
| `shell_spawn`'s `command` | the workspace check `cli` applies, same analyzer (`jaato_server/shared/plugins/command_containment.py`) | **refuses** — the spawn command IS a shell command |
