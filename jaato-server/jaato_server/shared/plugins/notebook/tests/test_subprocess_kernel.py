...
import os
import pytest
from jaato_server.shared.plugins.notebook.backends.subprocess_kernel import SubprocessKernelBackend
...
def test_tools_bridge_runs_in_trusted_permission_scope(tmp_path):
    from jaato_server.shared.ai_tool_runner import in_trusted_bridge_context
    be = SubprocessKernelBackend()
    be.initialize({"workspace_root": str(tmp_path)})
    seen = []
    be.set_tool_executor(
...
def test_session_contextvar_workspace_wins_over_plugin_config(tmp_path):
    ...
    from jaato_server.shared.session_context import set_workspace_root, reset_workspace_root
...
def test_pdeathsig_preexec_resolves_libc_at_module_level():
    ...
    # bare prctl syscall.
    from jaato_server.shared.plugins.notebook.backends import subprocess_kernel as sk
