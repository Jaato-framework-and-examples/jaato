...
def isolated_temp(monkeypatch, tmp_path):
    ...
    fake_temp.mkdir()
    monkeypatch.setattr(
        "jaato_server.shared.plugins.sandbox_utils.SYSTEM_TEMP_PATHS", [str(fake_temp)])
    return fake_temp
...
class TestEstablishContainment:
    ...
    def test_apparmor_wins_and_installs_no_hook(self, monkeypatch, tmp_path):
        monkeypatch.setattr(
            "jaato_server.shared.plugins.notebook.kernel_sandbox.apparmor_enforced_profile",
            lambda: "jaato-ws-x//child")
        installed = []
        monkeypatch.setattr(
            "jaato_server.shared.plugins.notebook.kernel_sandbox.install", installed.append)
...
class TestKernelRefusalFrames:
    ...
    def test_absent_allow_block_does_not_clear_the_policy(self, tmp_path,
        ...
        monkeypatch.setattr(
            "jaato_server.shared.plugins.notebook.kernel_sandbox.current_policy", lambda: pol)
