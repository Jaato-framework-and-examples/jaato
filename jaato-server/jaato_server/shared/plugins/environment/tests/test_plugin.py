...
from jaato_server.shared.terminal_caps import invalidate_cache as invalidate_terminal_cache
...
class TestEnvironmentPluginCustomAspects:
    """Tests for dynamic custom aspect registration."""

    def test_register_custom_aspect(self):
        """Custom aspects are queryable after registration."""
        plugin = EnvironmentPlugin()
        plugin.register_aspect(
            "my_aspect",
            lambda: {"key": "value"},
            description="test aspect",
        )
        result = json.loads(plugin._get_environment({"aspect": "my_aspect"}))
        assert result == {"key": "value"}

    def test_custom_aspect_in_valid_aspects(self):
        """Registered aspects appear in VALID_ASPECTS and tool schema enum."""
        plugin = EnvironmentPlugin()
        plugin.register_aspect("my_aspect", lambda: {}, description="test")
        assert "my_aspect" in plugin.VALID_ASPECTS
        schemas = plugin.get_tool_schemas()
        aspect_enum = schemas[0].parameters["properties"]["aspect"]["enum"]
        assert "my_aspect" in aspect_enum

    def test_custom_aspect_excluded_from_all(self):
        """Custom aspects are not included in aspect='all'."""
        plugin = EnvironmentPlugin()
        plugin.register_aspect("my_aspect", lambda: {"x": 1})
        result = json.loads(plugin._get_environment({"aspect": "all"}))
        assert "my_aspect" not in result

    def test_custom_aspect_description_in_schema(self):
        """Registered description appears in the tool schema."""
        plugin = EnvironmentPlugin()
        plugin.register_aspect("cluster", lambda: {}, description="cluster info")
        schemas = plugin.get_tool_schemas()
        desc = schemas[0].parameters["properties"]["aspect"]["description"]
        assert "'cluster' = cluster info" in desc
...
class TestConsumptionAspect:
    ...
    def _plugin_with_session(self, report=None):
        from jaato_server.shared.plugins.environment import EnvironmentPlugin
