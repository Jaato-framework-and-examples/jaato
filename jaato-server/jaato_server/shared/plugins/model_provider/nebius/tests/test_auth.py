"""Tests for Nebius API key validation and credential loading.

Parallels the Zhipu AI auth tests: verifies that quota / billing /
...
CLIENT_PATCH = "jaato_server.shared.plugins.model_provider.nebius.auth._create_validation_client"
...
class TestValidateApiKey:
    ...
    def test_success_returns_true(self):
        with patch(CLIENT_PATCH) as mock_create:
            ...
            valid, detail = validate_api_key("nbk-test-test")
    ...
    def test_429_returns_rate_limit_not_success(self):
        ...
        This was the Nebius-specific repro of Pattern B: a quota-exceeded
        response would previously return ``(True, "")`` and the caller
        would save the key as if it worked.
        """
        with patch(CLIENT_PATCH) as mock_create:
            mock_create.return_value = _mock_client(
    ...
    def test_uses_bearer_auth_header(self):
        """Validates that Nebius uses Authorization: Bearer (not x-api-key)."""
        with patch(CLIENT_PATCH) as mock_create:
            mock_create.return_value = _mock_client()
            validate_api_key("nbk-test-secret")
            call_args = mock_create.return_value.post.call_args
            headers = call_args[1]["headers"]
            assert headers["Authorization"] == "Bearer nbk-test-secret"
...
class TestTryLoadCredentialsWithReason:
    ...
    def test_home_tier_is_read_when_the_project_tier_is_empty(
        ...
        """``~/.jaato/nebius_auth.json`` answers when the workspace has none.
        ...
        """
        home_file = fake_home / ".jaato" / "nebius_auth.json"
        home_file.write_text(json.dumps({
            "api_key": "nebius-home-tier",
            "created_at": 1234567890,
        }))

        creds, reason = try_load_credentials_with_reason(workspace_path=str(tmp_path))
        assert creds is not None
        assert creds.api_key == "nebius-home-tier"
        assert reason is None

    def test_valid_file_loads_credentials(self, tmp_path):
        jaato_dir = tmp_path / ".jaato"
        jaato_dir.mkdir()
        auth_file = jaato_dir / "nebius_auth.json"
        auth_file.write_text(json.dumps({
            "api_key": "nbk-test-abc",
            "created_at": 1234567890,
        }))

        creds, reason = try_load_credentials_with_reason(workspace_path=str(tmp_path))
        assert creds is not None
        assert creds.api_key == "nbk-test-abc"
        assert reason is None
