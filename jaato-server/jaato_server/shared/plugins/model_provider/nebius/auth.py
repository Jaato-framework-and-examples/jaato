"""Authentication module for Nebius Token Factory.

Nebius uses API key authentication.
This module provides:
- Secure local storage of API keys
- Key validation against the Nebius endpoint
- Status checking

API keys are obtained from:
  https://tokenfactory.nebius.com/ → Settings → API Keys

Storage follows jaato convention:
1. Project .jaato/nebius_auth.json (project-specific)
2. Home ~/.jaato/nebius_auth.json (user-level default)
"""
...
@dataclass
class NebiusCredentials:
    """Stored Nebius credentials."""
    api_key: str
    ...
    @classmethod
    def from_dict(cls, data: dict) -> "NebiusCredentials":
        return cls(
...
def _get_token_storage_path(
    ...
    1. Project tier — ``<config_root>/nebius_auth.json`` when
       ``config_root`` is set, else ``<workspace>/.jaato/nebius_auth.json``.
    2. Home tier — ``~/.jaato/nebius_auth.json``.
    ...
    if effective_config_root:
        project_path = Path(effective_config_root).expanduser().resolve() / "nebius_auth.json"
    else:
        project_path = Path(workspace) / ".jaato" / "nebius_auth.json"
    home_path = Path.home() / ".jaato" / "nebius_auth.json"
...
def save_credentials(credentials: NebiusCredentials, workspace_path: Optional[str] = None) -> None:
...
def try_load_credentials_with_reason(
    workspace_path: Optional[str] = None,
    config_root: Optional[str] = None,
) -> Tuple[Optional[NebiusCredentials], Optional[str]]:
    """Load credentials and return a reason string when the load fails.

    Returns ``(credentials, reason)``:

    - ``(NebiusCredentials, None)`` — file loaded successfully.
    ...
    try:
        ...
        logger.warning("Failed to read Nebius credentials: %s", reason)
        return None, reason
    except json.JSONDecodeError as exc:
        reason = f"invalid JSON at {path}: {exc.msg} (line {exc.lineno}, col {exc.colno})"
        logger.warning("Failed to parse Nebius credentials: %s", reason)
        return None, reason

    try:
        return NebiusCredentials.from_dict(data), None
    except (KeyError, TypeError) as exc:
        reason = f"malformed credentials at {path}: missing or invalid field ({exc})"
        logger.warning("Malformed Nebius credentials: %s", reason)
        return None, reason
        ...
        logger.warning("Unexpected error loading Nebius credentials: %s", reason)
        return None, reason
...
def get_credential_file_path(
    ...
    Returns:
        String path like ``"~/.jaato/nebius_auth.json"`` or
        ``".jaato/nebius_auth.json"``, or None.
    """
    path = _get_token_storage_path(
        workspace_path=workspace_path, config_root=config_root,
    )
...
def validate_api_key(
    ...
    Args:
        api_key: Nebius API key to validate.
        base_url: Optional custom base URL (default: Nebius hosted API).
    ...
    Returns:
        ...
        - ``"server_error: <status>: <body>"`` — Nebius endpoint is
          temporarily unavailable (5xx).
        - ``"http_error: <status>: <body>"`` — any other unexpected
          status.
        - ``"network_error: <details>"`` — request never reached Nebius.
    """
    import httpx
...
def login_with_key(
    ...
    Returns:
        NebiusCredentials if successful, None if validation failed.
    """
    if on_message:
    ...
    if valid:
        ...
        if on_message:
            if detail.startswith("network_error"):
                on_message(
                    "Could not reach the Nebius API to validate your key. "
                    f"({detail})"
                )
            else:
