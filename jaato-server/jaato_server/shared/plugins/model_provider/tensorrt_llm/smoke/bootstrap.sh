...
# user fills in TENSORRT_LLM_HOST) is only created when it does not
...
#     from the profile as ${TENSORRT_LLM_HOST}.  Edit the workspace
#     .env after bootstrap.
#
...
After bootstrap, edit <workspace>/.env to set TENSORRT_LLM_HOST.
...
if [[ -f "$WORKSPACE/.env" ]]; then
    echo "==> Preserving existing $WORKSPACE/.env (edit it if TENSORRT_LLM_HOST needs to change)"
else
    cp -f "$SMOKE_DIR/.env.example" "$WORKSPACE/.env"
    echo "==> Created $WORKSPACE/.env from .env.example — edit TENSORRT_LLM_HOST to point at your endpoint"
fi
...
Bootstrap complete.  Next:
  ...
  1. Edit  $WORKSPACE/.env  to set TENSORRT_LLM_HOST.
