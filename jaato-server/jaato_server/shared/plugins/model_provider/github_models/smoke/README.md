...
| `GITHUB_TOKEN` | workspace `.env`, referenced from profile as `${GITHUB_TOKEN}` in `plugin_configs.github_models.api_key` | Credential varies per deployment, not per profile. Resolved at profile-load time via the framework's `${VAR}` substitution chain (`jaato_server/shared/plugins/subagent/config.py:_expand_string`). |
...
```bash
./jaato-server/jaato_server/shared/plugins/model_provider/github_models/smoke/bootstrap.sh
```
...
```bash
SMOKE=jaato-server/jaato_server/shared/plugins/model_provider/github_models/smoke
