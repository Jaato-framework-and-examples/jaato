"""Authentication module for the Moonshot AI Kimi API.

Moonshot AI Kimi uses API key (Bearer token) authentication.  This module provides:
- Secure local storage of API keys
- Key validation against the account balance endpoint (``GET /v1/users/me/balance``)
- Status checking

API keys are generated at https://platform.kimi.ai/console/api-keys

Storage follows jaato convention:
1. Project .jaato/kimi_auth.json (project-specific)
2. Home ~/.jaato/kimi_auth.json (user-level default)
"""
...
_FILENAME = "kimi_auth.json"


@dataclass
class KimiCredentials:
    """Stored Moonshot AI Kimi credentials."""
    ...
    @classmethod
    def from_dict(cls, data: dict) -> "KimiCredentials":
        return cls(
...
def _get_token_storage_path(
    ...
    1. Project tier — ``<config_root>/kimi_auth.json`` when
       ``config_root`` is set, else ``<workspace>/.jaato/kimi_auth.json``.
    2. Home tier — ``~/.jaato/kimi_auth.json``.
...
def save_credentials(credentials: KimiCredentials, workspace_path: Optional[str] = None) -> None:
...
def try_load_credentials_with_reason(
    ...
    try:
        ...
        logger.warning("Failed to read Moonshot AI Kimi credentials: %s", reason)
        return None, reason
    except json.JSONDecodeError as exc:
        reason = f"invalid JSON at {path}: {exc.msg} (line {exc.lineno}, col {exc.colno})"
        logger.warning("Failed to parse Moonshot AI Kimi credentials: %s", reason)
        return None, reason
    try:
        return KimiCredentials.from_dict(data), None
    except (KeyError, TypeError) as exc:
        reason = f"malformed credentials at {path}: missing or invalid field ({exc})"
        logger.warning("Malformed Moonshot AI Kimi credentials: %s", reason)
        return None, reason
        ...
        logger.warning("Unexpected error loading Moonshot AI Kimi credentials: %s", reason)
        return None, reason
...
def validate_api_key(
    ...
    Probes the account balance endpoint (``GET /v1/users/me/balance``) — no tokens are billed and the answer proves the
    ...
    test_url = f"{url.rstrip('/')}/users/me/balance"
    headers = {
...
def login_with_key(
    ...
    Returns:
        KimiCredentials if successful, None if validation failed.
    """
    if on_message:
    ...
    if not valid:
        if on_message:
            if detail.startswith("network_error"):
                on_message(f"Could not reach the Moonshot AI Kimi API to validate your key. ({detail})")
            else:
