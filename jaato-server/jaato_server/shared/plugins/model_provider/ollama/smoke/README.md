...
| `host` | workspace `.env` as `OLLAMA_HOST`, referenced from profile as `${OLLAMA_HOST}` | Endpoint detail varies per deployment (local vs LAN box vs proxy-fronted host), not per profile. Resolved at profile-load time via the framework's `${VAR}` substitution chain (`jaato_server/shared/plugins/subagent/config.py:_expand_string`). |
...
```bash
./jaato-server/jaato_server/shared/plugins/model_provider/ollama/smoke/bootstrap.sh
```
...
```bash
SMOKE=jaato-server/jaato_server/shared/plugins/model_provider/ollama/smoke
