"""Tool-calling smoke for the ollama provider.

Like ``smoke.py`` but uses the ``ollama-tools`` profile, which
...
tool schemas in the Anthropic shape the model expects, parses tool-call
