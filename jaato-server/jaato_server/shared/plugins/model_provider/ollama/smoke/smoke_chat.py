"""Chat smoke for the ollama provider — pure text round-trip.
...
local Ollama server (v0.14.0+) and round-trip a message via the
Anthropic-compatible API.  Tool-calling fidelity is exercised by
the sibling ``smoke_signal_completion.py`` (lifecycle tool only)
and ``smoke_tools.py`` (cli + signal_completion).
...
Prerequisites:
    ...
    3. ``host`` in the profile resolves (via ${OLLAMA_HOST} in workspace
       .env) to a reachable Ollama server, and ``model`` matches a model
       that ``ollama list`` reports as pulled.

Run:
    .venv/bin/jaato-server --ipc-socket /tmp/jaato.sock --daemon
    .venv/bin/python smoke_chat.py --workspace /tmp/jaato-ollama-smoke
    # or from inside the workspace:
    cd /tmp/jaato-ollama-smoke && .venv/bin/python /path/to/smoke_chat.py
"""
