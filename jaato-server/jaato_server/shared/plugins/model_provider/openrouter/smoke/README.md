...
| `api_key` | workspace `.env` as `JAATO_OPENROUTER_API_KEY`, referenced from profile as `${JAATO_OPENROUTER_API_KEY}` | The key is the per-user / per-deployment secret, not a property of the profile. Resolved at profile-load time via the framework's `${VAR}` substitution chain (`jaato_server/shared/plugins/subagent/config.py:_expand_string`). |
...
```bash
./jaato-server/jaato_server/shared/plugins/model_provider/openrouter/smoke/bootstrap.sh
```
...
```bash
SMOKE=jaato-server/jaato_server/shared/plugins/model_provider/openrouter/smoke
