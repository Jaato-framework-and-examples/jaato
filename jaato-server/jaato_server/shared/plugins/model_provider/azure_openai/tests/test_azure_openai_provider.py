...
def _no_stored_credentials(monkeypatch):
    monkeypatch.setattr(
        "jaato_server.shared.plugins.model_provider.azure_openai.auth.get_stored_api_key",
        lambda **_kw: None,
    )
    monkeypatch.setattr(
        "jaato_server.shared.plugins.model_provider.azure_openai.auth."
...
class TestVerifyAuth:
    ...
    def test_entra_passes_without_a_key_when_azure_identity_is_present(self):
        provider = AzureOpenAIProvider()
        with patch("jaato_server.shared.plugins.model_provider.azure_openai.auth."
