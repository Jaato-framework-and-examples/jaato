"""Tests for Doubleword API key validation and credential loading.

Parallels the Nebius/OVHcloud auth tests: verifies that quota / billing /
...
from ..auth import (
    DoublewordCredentials,
    try_load_credentials_with_reason,
    validate_api_key,
)
from ..env import DEFAULT_BASE_URL

CLIENT_PATCH = "jaato_server.shared.plugins.model_provider.doubleword.auth._create_validation_client"
...
class TestValidateApiKey:
    ...
    def test_success_returns_true(self):
        with patch(CLIENT_PATCH) as mock_create:
            ...
            valid, detail = validate_api_key("dw-test-test")
    ...
    def test_403_returns_true(self):
        """HTTP 403 means the key authenticated but lacks access to the probe
        model (Doubleword's documented 403: "Your API key does not have
        access to the requested model") — the key itself is proven valid."""
        with patch(CLIENT_PATCH) as mock_create:
            mock_create.return_value = _mock_client(403, body_text="no model access")
            valid, detail = validate_api_key("dw-test-test")
            assert valid is True

    def test_404_returns_true(self):
        """HTTP 404 (probe model rotated out of the catalog) still proves the
        key authenticated — Doubleword rejects a bad key with 401 regardless
        of the model."""
        with patch(CLIENT_PATCH) as mock_create:
    ...
    def test_429_returns_rate_limit_not_success(self):
        ...
        quota-exceeded keyed response must not save the key as if it
        worked."""
        with patch(CLIENT_PATCH) as mock_create:
            mock_create.return_value = _mock_client(
    ...
    def test_402_returns_payment_required(self):
        """HTTP 402 = insufficient credits (Doubleword's documented 402)."""
        with patch(CLIENT_PATCH) as mock_create:
            mock_create.return_value = _mock_client(402, body_text="Insufficient credits")
            valid, detail = validate_api_key("dw-test-test")
            assert valid is False
    ...
    def test_uses_bearer_auth_header(self):
        """Validates that Doubleword uses Authorization: Bearer."""
        with patch(CLIENT_PATCH) as mock_create:
            mock_create.return_value = _mock_client()
            validate_api_key("dw-test-secret")
            call_args = mock_create.return_value.post.call_args
            headers = call_args[1]["headers"]
            assert headers["Authorization"] == "Bearer dw-test-secret"
...
class TestTryLoadCredentialsWithReason:
    ...
    def test_valid_file_loads_credentials(self, tmp_path):
        ...
        jaato_dir.mkdir()
        auth_file = jaato_dir / "doubleword_auth.json"
        ...
        creds, reason = try_load_credentials_with_reason(workspace_path=str(tmp_path))
        assert creds is not None
        assert creds.api_key == "dw-test-abc"
        assert reason is None
