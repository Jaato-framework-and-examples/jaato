"""Authentication module for the Doubleword Inference API.

Doubleword uses API key (Bearer token) authentication.
This module provides:
- Secure local storage of API keys
- Key validation against the inference API
- Status checking

API keys are generated in the Doubleword dashboard:
  https://app.doubleword.ai/api-keys

Storage follows jaato convention:
1. Project .jaato/doubleword_auth.json (project-specific)
2. Home ~/.jaato/doubleword_auth.json (user-level default)
"""
...
@dataclass
class DoublewordCredentials:
    """Stored Doubleword credentials."""
    api_key: str
    ...
    @classmethod
    def from_dict(cls, data: dict) -> "DoublewordCredentials":
        return cls(
...
def _get_token_storage_path(
    ...
    1. Project tier — ``<config_root>/doubleword_auth.json`` when
       ``config_root`` is set, else ``<workspace>/.jaato/doubleword_auth.json``.
    2. Home tier — ``~/.jaato/doubleword_auth.json``.
    ...
    if effective_config_root:
        project_path = Path(effective_config_root).expanduser().resolve() / "doubleword_auth.json"
    else:
        project_path = Path(workspace) / ".jaato" / "doubleword_auth.json"
    home_path = Path.home() / ".jaato" / "doubleword_auth.json"
...
def save_credentials(credentials: DoublewordCredentials, workspace_path: Optional[str] = None) -> None:
...
def load_credentials(
    workspace_path: Optional[str] = None,
    config_root: Optional[str] = None,
) -> Optional[DoublewordCredentials]:
...
def try_load_credentials_with_reason(
    workspace_path: Optional[str] = None,
    config_root: Optional[str] = None,
) -> Tuple[Optional[DoublewordCredentials], Optional[str]]:
    """Load credentials and return a reason string when the load fails.

    Returns ``(credentials, reason)``:

    - ``(DoublewordCredentials, None)`` — file loaded successfully.
    ...
    try:
        ...
        logger.warning("Failed to read Doubleword credentials: %s", reason)
        return None, reason
    except json.JSONDecodeError as exc:
        reason = f"invalid JSON at {path}: {exc.msg} (line {exc.lineno}, col {exc.colno})"
        logger.warning("Failed to parse Doubleword credentials: %s", reason)
        return None, reason

    try:
        return DoublewordCredentials.from_dict(data), None
    except (KeyError, TypeError) as exc:
        reason = f"malformed credentials at {path}: missing or invalid field ({exc})"
        logger.warning("Malformed Doubleword credentials: %s", reason)
        return None, reason
        ...
        logger.warning("Unexpected error loading Doubleword credentials: %s", reason)
        return None, reason
...
def get_credential_file_path(
    ...
    Returns:
        String path like ``"~/.jaato/doubleword_auth.json"`` or
        ``".jaato/doubleword_auth.json"``, or None.
    """
    path = _get_token_storage_path(
        workspace_path=workspace_path, config_root=config_root,
    )
...
def _create_validation_client():
    ...
    from jaato_server.shared.http.proxy import get_httpx_client
...
def validate_api_key(
    ...
    Args:
        api_key: Doubleword API key to validate.
        base_url: Optional custom base URL (default: the hosted gateway).

    Returns:
        A ``(valid, detail)`` tuple.  ``valid`` is True only when the
        request authenticates: 2xx, 400 "bad request", 403 "no model
        access", or 404 "model not found" (Doubleword rejects a bad key
        with 401 regardless of the model, so reaching model routing
        proves the key — and the probe model may rotate out of the
        catalog).  Other non-auth errors do not masquerade as success.
        ``detail`` carries a structured code:
        ...
        - ``"payment_required: <status>: <body>"`` — account balance
          below zero (402).  Key was NOT saved.
    ...
    # Qwen3.5 4B is a small, cheap catalog model; a 403 (key lacks model
    # access) or 404 (model rotated out) still proves the key
    # authenticated (Doubleword answers 401 for a bad key before model
    # routing).
    body = {
    ...
    if 200 <= status < 300 or status in (400, 403, 404):
        return (True, "")
...
def login_with_key(
    ...
    Returns:
        DoublewordCredentials if successful, None if validation failed.
    """
    if on_message:
    ...
    if valid:
        credentials = DoublewordCredentials(
            api_key=api_key,
        ...
        if on_message:
            if detail.startswith("network_error"):
                on_message(
                    "Could not reach the Doubleword API to "
