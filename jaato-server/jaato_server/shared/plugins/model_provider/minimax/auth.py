"""Authentication module for the MiniMax API.

MiniMax uses API key (Bearer token) authentication.  This module provides:
- Secure local storage of API keys
- Key validation against the ``GET /v1/models`` catalog
- Status checking

API keys are generated at https://platform.minimax.io/ (Account → API Keys)

Storage follows jaato convention:
1. Project .jaato/minimax_auth.json (project-specific)
2. Home ~/.jaato/minimax_auth.json (user-level default)
"""
...
_FILENAME = "minimax_auth.json"


@dataclass
class MiniMaxCredentials:
    """Stored MiniMax credentials."""
    ...
    @classmethod
    def from_dict(cls, data: dict) -> "MiniMaxCredentials":
        return cls(
...
def _get_token_storage_path(
    ...
    1. Project tier — ``<config_root>/minimax_auth.json`` when
       ``config_root`` is set, else ``<workspace>/.jaato/minimax_auth.json``.
    2. Home tier — ``~/.jaato/minimax_auth.json``.
...
def save_credentials(credentials: MiniMaxCredentials, workspace_path: Optional[str] = None) -> None:
...
def try_load_credentials_with_reason(
    workspace_path: Optional[str] = None,
    config_root: Optional[str] = None,
) -> Tuple[Optional[MiniMaxCredentials], Optional[str]]:
    """Load credentials and return a reason string when the load fails.

    Returns ``(credentials, reason)``:

    - ``(MiniMaxCredentials, None)`` — file loaded successfully.
    ...
    try:
        ...
        logger.warning("Failed to read MiniMax credentials: %s", reason)
        return None, reason
    except json.JSONDecodeError as exc:
        reason = f"invalid JSON at {path}: {exc.msg} (line {exc.lineno}, col {exc.colno})"
        logger.warning("Failed to parse MiniMax credentials: %s", reason)
        return None, reason
    try:
        return MiniMaxCredentials.from_dict(data), None
    except (KeyError, TypeError) as exc:
        reason = f"malformed credentials at {path}: missing or invalid field ({exc})"
        logger.warning("Malformed MiniMax credentials: %s", reason)
        return None, reason
        ...
        logger.warning("Unexpected error loading MiniMax credentials: %s", reason)
        return None, reason
...
def validate_api_key(
    ...
    Probes the ``GET /v1/models`` catalog — no tokens are billed and the answer proves the
...
def login_with_key(
    ...
    Returns:
        MiniMaxCredentials if successful, None if validation failed.
    """
    if on_message:
    ...
    if not valid:
        if on_message:
            if detail.startswith("network_error"):
                on_message(f"Could not reach the MiniMax API to validate your key. ({detail})")
            else:
