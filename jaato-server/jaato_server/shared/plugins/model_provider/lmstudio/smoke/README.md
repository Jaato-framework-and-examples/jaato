...
| `host` | workspace `.env` as `LMSTUDIO_HOST`, referenced from profile as `${LMSTUDIO_HOST}` | Endpoint detail varies per deployment, not per profile. Resolved at profile-load time via the framework's `${VAR}` substitution chain (`jaato_server/shared/plugins/subagent/config.py:_expand_string`). |
...
```bash
./jaato-server/jaato_server/shared/plugins/model_provider/lmstudio/smoke/bootstrap.sh
```
...
```bash
SMOKE=jaato-server/jaato_server/shared/plugins/model_provider/lmstudio/smoke
