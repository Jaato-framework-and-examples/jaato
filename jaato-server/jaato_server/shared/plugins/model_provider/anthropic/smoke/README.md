...
Three end-to-end smokes for the `anthropic` provider, layered from simplest to most demanding:

| # | Smoke | What it validates | Profile | Harness |
|---|---|---|---|---|
| 1 | **Chat** | Provider wire — daemon can reach `api.anthropic.com` (or the configured base URL) and round-trip a Messages API call. **No tools, no `signal_completion`**: the profile declares no `completion_payload_schema` so `signal_completion` is hidden (2026-06-07 schema gate). The turn ends naturally when the model emits text without function calls. | `anthropic-chat` | `smoke_chat.py` |
| 2 | **signal_completion** | Lifecycle — schema-driven completion contract. Profile declares a non-trivial 3-field schema (summary + status + word_count, 2 required). Model must acknowledge in text then call `signal_completion` with a schema-valid payload. Tests both the Anthropic tool-use wire AND `jsonschema.validate` end-to-end. | `anthropic-signal_completion` | `smoke_signal_completion.py` |
| 3 | **Tools** | Full tool-calling — `cli` plugin (one shell tool call) followed by `signal_completion` (same schema as #2). Exercises the Anthropic tool-use shape with multiple tool surfaces. The `permission` plugin is server-wired automatically (no need to list it in `plugins`), but its policy is set via `plugin_configs.permission`. | `anthropic-tools` | `smoke_tools.py` |
...
the Anthropic tool-use serialization path or a model-fidelity gap on
structured output. If **tools** is red but the first two are green, the
issue is the `cli` tool's schema or chained tool-result handling.

Claude Sonnet / Opus models are generally strong on tool-calling and
structured output, so failures in tests 2-3 typically point at the
framework wire rather than model fidelity (unlike weaker self-hosted
models where 2-3 can fail purely on the model's side).
...
└── .jaato.example/                          # workspace .jaato/ template
    ├── profiles/
    │   ├── anthropic-chat.yaml              # no schema → signal_completion hidden
    │   ├── anthropic-signal_completion.yaml # 3-field schema, plugins:[]
    │   └── anthropic-tools.yaml             # 3-field schema, plugins:[cli]
    └── agents/
        ├── anthropic-chat.md                # text-only responder
        ├── anthropic-signal_completion.md   # acknowledge + payload
        └── anthropic-tools.md               # tool call + acknowledge + payload
```
...
```bash
./jaato-server/jaato_server/shared/plugins/model_provider/anthropic/smoke/bootstrap.sh
```
...
```bash
SMOKE=jaato-server/jaato_server/shared/plugins/model_provider/anthropic/smoke
...
| 1 with `NudgeExhausted: Agent loop exhausted N completion nudges` | The model responded with text but didn't call `signal_completion` to end the turn. **The wire worked** — the smoke validates provider connectivity, and a coherent text reply proves the wire end-to-end. NudgeExhausted on Claude is rare; capable models (Claude Sonnet 4.5, GPT-4o) follow the persona's `signal_completion` instruction cleanly. If you hit this on Claude, check whether the persona prose was inadvertently weakened. |
