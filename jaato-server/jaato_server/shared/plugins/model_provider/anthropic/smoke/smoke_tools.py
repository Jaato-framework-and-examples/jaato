"""Tool-calling smoke for the anthropic provider.

Test 3 of 3 (smoke_chat / smoke_signal_completion / smoke_tools):
uses the ``anthropic-tools`` profile, which exposes the ``cli``
...
provider serializes tool schemas in the Anthropic tool-use shape
the API expects, parses tool-call arguments correctly, round-trips
the tool result back into the conversation, AND that the same wire
shape carries the lifecycle ``signal_completion`` tool to a
schema-driven termination.  If chat-only ``smoke_chat.py`` is green
but this fails, the bug is in the tool-call path (schema
serialization, argument parsing, or the model's own tool-calling
fidelity), not the wire.  If ``smoke_signal_completion.py`` is also
green but this fails, the bug is specifically in the
multiple-tool-surface composition.
