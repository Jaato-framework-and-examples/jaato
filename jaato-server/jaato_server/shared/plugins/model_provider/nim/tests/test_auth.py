"""Tests for NIM API key validation and credential loading.
...
CLIENT_PATCH = "jaato_server.shared.plugins.model_provider.nim.auth._create_validation_client"
...
class TestValidateApiKey:
    ...
    def test_success_returns_true(self):
        with patch(CLIENT_PATCH) as mock_create:
            ...
            valid, detail = validate_api_key("nvapi-test")
    ...
    def test_429_returns_rate_limit_not_success(self):
        ...
        This was the NIM-specific repro of Pattern B: a quota-exceeded
    ...
    def test_uses_bearer_auth_header(self):
        """Validates that NIM uses Authorization: Bearer (not x-api-key)."""
        with patch(CLIENT_PATCH) as mock_create:
            mock_create.return_value = _mock_client()
            validate_api_key("nvapi-secret")
...
class TestTryLoadCredentialsWithReason:
    ...
    def test_home_tier_is_read_when_the_project_tier_is_empty(
        ...
        """``~/.jaato/nim_auth.json`` answers when the workspace has none.
        ...
        """
        home_file = fake_home / ".jaato" / "nim_auth.json"
        home_file.write_text(json.dumps({
            "api_key": "nvapi-home-tier",
            "created_at": 1234567890,
        }))

        creds, reason = try_load_credentials_with_reason(workspace_path=str(tmp_path))
        assert creds is not None
        assert creds.api_key == "nvapi-home-tier"
        assert reason is None

    def test_valid_file_loads_credentials(self, tmp_path):
        jaato_dir = tmp_path / ".jaato"
        jaato_dir.mkdir()
        auth_file = jaato_dir / "nim_auth.json"
        ...
        creds, reason = try_load_credentials_with_reason(workspace_path=str(tmp_path))
        assert creds is not None
        assert creds.api_key == "nvapi-abc"
        assert reason is None
