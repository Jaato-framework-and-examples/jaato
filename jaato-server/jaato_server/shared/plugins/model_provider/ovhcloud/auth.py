"""Authentication module for OVHcloud AI Endpoints.

OVHcloud AI Endpoints uses API key (Bearer token) authentication.
This module provides:
- Secure local storage of API keys
- Key validation against the AI Endpoints gateway
- Status checking

API keys are created in the OVHcloud Manager:
  Public Cloud → AI & Machine Learning → AI Endpoints → API keys

Storage follows jaato convention:
1. Project .jaato/ovhcloud_auth.json (project-specific)
2. Home ~/.jaato/ovhcloud_auth.json (user-level default)
"""
...
class OVHcloudCredentials:
    ...
    @classmethod
    def from_dict(cls, data: dict) -> "OVHcloudCredentials":
        return cls(
...
def _get_token_storage_path(
    ...
    1. Project tier — ``<config_root>/ovhcloud_auth.json`` when
       ``config_root`` is set, else ``<workspace>/.jaato/ovhcloud_auth.json``.
    2. Home tier — ``~/.jaato/ovhcloud_auth.json``.
    ...
    if effective_config_root:
        project_path = Path(effective_config_root).expanduser().resolve() / "ovhcloud_auth.json"
    else:
        project_path = Path(workspace) / ".jaato" / "ovhcloud_auth.json"
    home_path = Path.home() / ".jaato" / "ovhcloud_auth.json"
...
def save_credentials(credentials: OVHcloudCredentials, workspace_path: Optional[str] = None) -> None:
...
def try_load_credentials_with_reason(
    workspace_path: Optional[str] = None,
    config_root: Optional[str] = None,
) -> Tuple[Optional[OVHcloudCredentials], Optional[str]]:
    """Load credentials and return a reason string when the load fails.

    Returns ``(credentials, reason)``:

    - ``(OVHcloudCredentials, None)`` — file loaded successfully.
    ...
    try:
        ...
        logger.warning("Failed to read OVHcloud credentials: %s", reason)
        return None, reason
    except json.JSONDecodeError as exc:
        reason = f"invalid JSON at {path}: {exc.msg} (line {exc.lineno}, col {exc.colno})"
        logger.warning("Failed to parse OVHcloud credentials: %s", reason)
        return None, reason

    try:
        return OVHcloudCredentials.from_dict(data), None
    except (KeyError, TypeError) as exc:
        reason = f"malformed credentials at {path}: missing or invalid field ({exc})"
        logger.warning("Malformed OVHcloud credentials: %s", reason)
        return None, reason
        ...
        logger.warning("Unexpected error loading OVHcloud credentials: %s", reason)
        return None, reason
...
def get_credential_file_path(
    ...
    Returns:
        String path like ``"~/.jaato/ovhcloud_auth.json"`` or
        ``".jaato/ovhcloud_auth.json"``, or None.
    """
    path = _get_token_storage_path(
        workspace_path=workspace_path, config_root=config_root,
    )
...
def validate_api_key(
    ...
    Args:
        api_key: OVHcloud AI Endpoints API key to validate.
        base_url: Optional custom base URL (default: the unified gateway).

    Returns:
        A ``(valid, detail)`` tuple.  ``valid`` is True only when the
        request authenticates: 2xx, 400 "bad request", or 404 "model not
        found" (OVHcloud rejects a bad key with 401 regardless of the
        model, so reaching model routing proves the key — and the probe
        model may rotate out of the catalog).  Other non-auth errors do
        not masquerade as success.  ``detail`` carries a structured code:
        ...
        - ``"server_error: <status>: <body>"`` — the gateway is
          temporarily unavailable (5xx).
        - ``"http_error: <status>: <body>"`` — any other unexpected
          status.
        - ``"network_error: <details>"`` — request never reached OVHcloud.
    """
    import httpx
    ...
    # gpt-oss-20b is a small, cheap catalog model; a 404 (model rotated
    # out) still proves the key authenticated.
    body = {
        "model": "gpt-oss-20b",
        "max_tokens": 1,
...
def login_with_key(
    ...
    Args:
        api_key: OVHcloud AI Endpoints API key.
        base_url: Optional custom base URL.
        on_message: Callback for status messages.

    Returns:
        OVHcloudCredentials if successful, None if validation failed.
    """
    if on_message:
    ...
    if valid:
        credentials = OVHcloudCredentials(
            api_key=api_key,
        ...
        if on_message:
            if detail.startswith("network_error"):
                on_message(
                    "Could not reach the OVHcloud AI Endpoints API to "
