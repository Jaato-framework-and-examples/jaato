...
class TestEnvironment:
    ...
    def test_resolve_api_key_from_vendor_env(self):
        ...
        with patch.dict(
            ...
            with patch(
                "jaato_server.shared.plugins.model_provider.ovhcloud.auth.get_stored_api_key",
                return_value=None,
            ):
...
class TestAuthentication:
    ...
    @patch("jaato_server.shared.plugins.model_provider._openai_compat.base.get_openai_client_class")
    ...
    def test_initialize_anonymous_env_opt_in(self, mock_client_class):
        ...
        provider = OVHcloudProvider()
        with patch(
            "jaato_server.shared.plugins.model_provider.ovhcloud.env.resolve_api_key",
            return_value=None,
        ):
            provider.initialize(ProviderConfig(extra={"context_length": 131072}))
...
class TestVerifyAuth:
    ...
    def test_verify_auth_honors_profile_extra_api_key(self):
        ...
        with patch.dict("os.environ", {}, clear=True):
            with patch(
                "jaato_server.shared.plugins.model_provider.ovhcloud.auth"
    ...
    def test_verify_auth_surfaces_broken_credentials(self):
        ...
        with patch.dict("os.environ", {}, clear=True):
            with patch(
                "jaato_server.shared.plugins.model_provider.ovhcloud.auth.try_load_credentials_with_reason",
...
class TestApiParams:
    ...
    def _provider(self, api_params):
        with patch(
            "jaato_server.shared.plugins.model_provider._openai_compat.base.get_openai_client_class"
        ) as mc:
