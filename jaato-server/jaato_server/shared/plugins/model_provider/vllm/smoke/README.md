...
| `host` | workspace `.env` as `VLLM_HOST`, referenced from profile as `${VLLM_HOST}` | Endpoint detail varies per deployment, not per profile. Resolved at profile-load time via the framework's `${VAR}` substitution chain (`jaato_server/shared/plugins/subagent/config.py:_expand_string`). |
...
```bash
./jaato-server/jaato_server/shared/plugins/model_provider/vllm/smoke/bootstrap.sh
```
...
```bash
SMOKE=jaato-server/jaato_server/shared/plugins/model_provider/vllm/smoke
