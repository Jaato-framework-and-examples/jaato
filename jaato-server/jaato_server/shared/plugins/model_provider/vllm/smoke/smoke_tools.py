"""Tool-calling smoke for the vllm provider.

Like ``smoke.py`` but uses the ``vllm-tools`` profile, which exposes
the ``cli`` plugin with default-allow permissions.  The model should
make exactly one ``cli_based_tool`` call (``ls /tmp``) and then respond
...
The vLLM server must have been launched with
``--enable-auto-tool-choice --tool-call-parser <name>`` matching the
model family (e.g. ``hermes`` for Qwen2.5, ``llama3_json`` for Llama
3.1).  Without those flags, vLLM will not parse the model's tool-call
output and the model's tool intent will appear in the text channel
instead.
