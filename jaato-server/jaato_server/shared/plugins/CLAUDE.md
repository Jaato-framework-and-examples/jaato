...
below is an AST scan of `jaato_server/shared/plugins/`, so a distribution shipping
...
The build-fail gate is in `jaato_server/shared/tests/test_plugin_tier_partition.py`:
...
(`jaato_server/shared/plugins/runner_forwarding.py`) — same wrap-point pattern,
opposite direction.
...
**Why this matters:** The daemon (`jaato_server/server/__main__.py`) has two command sources:
...
1. Add a `TRAIT_*` constant in `jaato_server/shared/plugins/model_provider/types.py` with a
...
`jaato_server/shared/plugins/path_safety.py`, which resolves once and proves the descriptor
...
from jaato_server.shared.session_context import get_session_env  # in-tree (same function)
...
`jaato_sdk` and `jaato_sdk.plugins.base`) and `jaato_server.shared.session_context`
...
Model provider plugins (`jaato_server/shared/plugins/model_provider/<name>/provider.py`) must
...
3. Session plugins → `session_manager.handle_request()` → `jaato_server.server.execute_command()`
