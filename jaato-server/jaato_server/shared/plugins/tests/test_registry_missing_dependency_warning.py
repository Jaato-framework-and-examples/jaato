...
def test_module_not_found_emits_concise_warning_not_traceback(
    ...
    with patch(
        "jaato_server.shared.plugins.registry.pkgutil.iter_modules",
        return_value=_one_fake_module("interactive_shell"),
    ), patch(
        "jaato_server.shared.plugins.registry.importlib.import_module",
        side_effect=_selective_import("interactive_shell", missing),
    ):
        with caplog.at_level(logging.DEBUG, logger="jaato_server.shared.plugins.registry"):
...
def test_non_import_error_still_logs_full_traceback_error() -> None:
    ...
    with patch(
        ...
        with patch("jaato_server.shared.plugins.registry.logger") as mock_logger:
            discovered = registry._discover_via_directory(plugin_kind="tool")
...
def test_install_advice_names_the_extra_that_ships_the_module():
    from jaato_server.shared.plugins.registry import _install_advice
...
def test_install_advice_never_raises(monkeypatch):
    ...
    import jaato_server.shared.scaffold.dependencies as deps
    from jaato_server.shared.plugins.registry import _install_advice
