...
def _discover(registry: PluginRegistry, *eps: Any) -> List[str]:
    ...
    with patch(
        "jaato_server.shared.plugins.registry.importlib.metadata.entry_points",
...
def test_foreign_entry_point_claiming_builtin_name_is_refused_before_load(
    ...
    with caplog.at_level(logging.WARNING, logger="jaato_server.shared.plugins.registry"):
...
def test_framework_declaring_its_own_plugins_is_silent(
    ...
    ep = _make_ep(
        "permission",
        "jaato_server.shared.plugins.permission:create_plugin",
...
def test_allowlist_never_blocks_the_builtin_package(
    ...
    ep = _make_ep("cli", "jaato_server.shared.plugins.cli:create_plugin", distribution="jaato-server")
    registry = PluginRegistry()
...
def test_a_builtin_reaching_the_registry_twice_is_quiet(
    ...
    registry._plugin_sources["session"] = PluginOrigin(
        name="session",
        via="registered",
        module="jaato_server.shared.plugins.session.file_session",
    )
...
def test_provenance_is_recorded_for_every_registered_plugin() -> None:
    ...
    assert source.module.startswith("jaato_server.shared.plugins.cli")
    assert "jaato_server.shared.plugins.cli" in source.describe()
...
@pytest.mark.parametrize(
    ...
    [
        ("cli", "jaato_server.shared.plugins.cli", "builtin"),
...
def test_scaffold_report_marks_out_of_tree_plugins() -> None:
    ...
    introspect._stamp_origin(
        builtin,
        PluginOrigin(name="cli", via="entry_point", module="jaato_server.shared.plugins.cli"),
    )
