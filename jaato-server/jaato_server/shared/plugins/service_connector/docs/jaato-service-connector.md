...
Uses `httpx` (preferred) or `requests` (fallback). Respects proxy configuration via `get_httpx_kwargs()` / `get_requests_kwargs()` from `jaato_server.shared.http`. When `ssl_trusted` is set on the service config, SSL verification is skipped. When `proxy_bypass` is set, proxy kwargs are omitted.
...
3. **Proxy settings** — read from `jaato_server.shared.http` module (respects `HTTP_PROXY`, `HTTPS_PROXY`, `NO_PROXY`)
