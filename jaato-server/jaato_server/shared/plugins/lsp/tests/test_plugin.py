...
class TestAtexitReapJdtls:
    ...
    def test_sigkills_each_tracked_pid(self):
        ...
        with patch("jaato_server.shared.plugins.lsp.plugin.os.kill") as mock_kill:
    ...
    def test_swallows_kill_errors(self):
        ...
        with patch(
            "jaato_server.shared.plugins.lsp.plugin.os.kill",
    ...
    def test_register_is_idempotent(self):
        ...
        plugin = LSPToolPlugin()
        with patch("jaato_server.shared.plugins.lsp.plugin.atexit.register") as mock_reg:
...
class TestDaemonProcessSuppressesLSP:
    ...
    def test_detects_daemon_via_main_package(self, monkeypatch):
        import jaato_server.shared.plugins.lsp.plugin as lspmod
    ...
    def test_detects_runner_via_main_package(self, monkeypatch):
        import jaato_server.shared.plugins.lsp.plugin as lspmod
        monkeypatch.setattr(sys.modules["__main__"], "__package__", "jaato_server.server.runner", raising=False)
    ...
    def test_argv_fallback_detects_daemon(self, monkeypatch):
        ...
        monkeypatch.setattr(sys, "argv", ["/x/jaato-server/jaato_server/server/__main__.py", "--daemon"])
        assert lspmod._running_in_daemon_process() is True
