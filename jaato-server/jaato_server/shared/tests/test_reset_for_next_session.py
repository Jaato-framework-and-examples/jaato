...
class TestThinkingPluginReset:
    ...
    def _make_plugin(self):
        from jaato_server.shared.plugins.thinking.plugin import ThinkingPlugin
...
class TestAcrossSessionByDesignPlugins:
    ...
    def test_artifact_tracker_reset_preserves_registry(self):
        ...
        Daniel-corrected 2026-05-20."""
        from jaato_server.shared.plugins.artifact_tracker.plugin import ArtifactTrackerPlugin
    ...
    def test_todo_reset_preserves_plan_ids(self):
        ...
        from jaato_server.shared.plugins.todo.plugin import TodoPlugin
        plugin = TodoPlugin()
...
class TestSandboxManagerPluginReset:
    ...
    def test_reset_clears_session_id_and_paths(self):
        from jaato_server.shared.plugins.sandbox_manager.plugin import SandboxManagerPlugin
...
class TestWaypointPluginReset:
    ...
    def test_reset_clears_session_state_and_callbacks(self):
        from jaato_server.shared.plugins.waypoint.plugin import WaypointPlugin
...
class TestImplementationsDocumentLitmusTest:
    ...
    @pytest.mark.parametrize("plugin_module,plugin_class_name,expected_marker", [
        ...
        ("jaato_server.shared.plugins.session.file_session", "FileSessionPlugin", "litmus test"),
        ("jaato_server.shared.plugins.permission.plugin", "PermissionPlugin", "litmus test"),
        ("jaato_server.shared.plugins.thinking.plugin", "ThinkingPlugin", "litmus test"),
        ("jaato_server.shared.plugins.todo.plugin", "TodoPlugin", "litmus test"),
        ("jaato_server.shared.plugins.clarification.plugin", "ClarificationPlugin", "litmus test"),
        ("jaato_server.shared.plugins.lsp.plugin", "LSPToolPlugin", "cascade-sharing target"),
        ("jaato_server.shared.plugins.memory.plugin", "MemoryPlugin", "litmus test"),
        ("jaato_server.shared.plugins.artifact_tracker.plugin", "ArtifactTrackerPlugin", "litmus test"),
        # Phase 1b — 5 plugins (server 0.6.143)
        ("jaato_server.shared.plugins.mcp.plugin", "MCPToolPlugin", "cascade-sharing target"),
        ("jaato_server.shared.plugins.interactive_shell.plugin", "InteractiveShellPlugin", "litmus test"),
        ("jaato_server.shared.plugins.sandbox_manager.plugin", "SandboxManagerPlugin", "litmus test"),
        ("jaato_server.shared.plugins.subagent.plugin", "SubagentPlugin", "litmus test"),
        ("jaato_server.shared.plugins.waypoint.plugin", "WaypointPlugin", "litmus test"),
    ])
