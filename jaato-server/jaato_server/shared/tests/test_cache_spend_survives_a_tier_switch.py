...
from jaato_server.shared.model_tiers import ModelTierConfig, TierEntry
...
REVERSIONS = [
    ...
    Reversion(
        target=("jaato-server/jaato_server/shared/plugins/model_provider/openrouter"
                "/converters.py"),
...
SESSION_SRC = (ROOT / "jaato-server" / "jaato_server" / "shared"
...
class TestTheChainCarriesIt:
    ...
    @pytest.mark.parametrize("path,what", [
        ("jaato-server/jaato_server/shared/jaato_client.py", "the facade driver"),
    ...
    def test_the_runner_puts_it_on_the_wire(self):
        ...
        keys = self._dict_literal_keys("jaato-server/jaato_server/server/runner/rpc.py")
        for field in self.FIELDS:
    ...
    def test_the_runner_reads_it_from_turn_accounting(self):
        ...
        reads = self._get_string_args("jaato-server/jaato_server/server/runner/rpc.py")
    ...
    def test_the_daemon_unpacks_it_from_the_wire(self):
        keys = self._get_string_args("jaato-server/jaato_server/server/core.py")
        for field in self.FIELDS:
            assert field in keys, (
    ...
    def test_the_daemon_passes_it_to_the_usage_builder(self):
        forwarded = self._keywords_of_calls_to(
            "jaato-server/jaato_server/server/core.py", "_build_usage")
        for field in self.FIELDS:
            assert field in forwarded, (
