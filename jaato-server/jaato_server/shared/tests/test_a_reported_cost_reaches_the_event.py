...
def test_cost_accumulates_across_a_turns_responses():
    ...
    """
    src = pathlib.Path(
        "jaato-server/jaato_server/shared/jaato_session.py").read_text(encoding="utf-8")
...
@pytest.mark.parametrize("path,what", [
    ("jaato-server/jaato_server/shared/plugins/subagent/ui_hooks.py", "the hook signature"),
    ("jaato-server/jaato_server/server/runner/rpc.py", "the RPC payload"),
    ("jaato-server/jaato_server/server/core.py", "the daemon-side unpack + build"),
...
def test_the_event_builder_is_actually_given_the_override():
    ...
    """
    src = pathlib.Path("jaato-server/jaato_server/server/core.py").read_text(encoding="utf-8")
    tree = ast.parse(src)
