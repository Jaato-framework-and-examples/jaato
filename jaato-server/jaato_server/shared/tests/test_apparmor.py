...
class TestAvailability:
    ...
    def test_not_available_without_apparmor_parser(self, manager):
        with patch("jaato_server.server.apparmor.platform.system", return_value="Linux"), \
             patch("jaato_server.server.apparmor.shutil.which", return_value=None):
            manager._available = None
            assert manager.is_available() is False

    def test_not_available_without_kernel_module(self, manager):
        with patch("jaato_server.server.apparmor.platform.system", return_value="Linux"), \
             patch("jaato_server.server.apparmor.shutil.which", return_value="/usr/sbin/apparmor_parser"), \
             patch("jaato_server.server.apparmor.Path.exists", return_value=False):
            manager._available = None
            assert manager.is_available() is False
...
class TestRenderProfile:
    ...
    def test_allows_reading_user_tier_services(self, manager):
        ...
        """
        from jaato_server.shared.plugins.service_connector.plugin import ServiceConnectorPlugin
    ...
    def test_make_tool_confine_context_yields_subprofile_path(self):
        ...
        """
        from jaato_server.server.apparmor import make_tool_confine_context
...
class TestMakeChildTransitionCallback:
    ...
    def test_writes_changeprofile_target_to_attr_current(self, tmp_path, monkeypatch):
        ...
        from jaato_server.server import apparmor as ap
...
class TestMakeConfineContext:
    def test_returns_callable(self):
        from jaato_server.server.apparmor import make_confine_context
    ...
    def test_confine_unavailable_profile_no_raise(self):
        ...
        from jaato_server.server.apparmor import apparmor_confine
...
class TestApparmorConfineDefensiveReset:
    ...
    def test_exit_restoration_failure_logged_at_error(self, caplog):
        ...
        with caplog.at_level("ERROR", logger="jaato_server.server.apparmor"):
            with patch("builtins.open", side_effect=fake_open):
                with apparmor_confine("jaato-ws-test"):
                    pass
...
class TestProvisionProfile:
    def test_writes_and_loads_profile(self, manager, profile_dir):
        manager._available = True
        with patch("jaato_server.server.apparmor.subprocess.run") as mock_run:
...
class TestReferenceAuthorizer:
    ...
    def test_authorize_delegates_to_manager(self, manager):
        from jaato_server.server.apparmor import ReferenceAuthorizer
...
class TestGCApparmorRules:
    ...
    def test_resolver_unions_gc_rules_when_profile_gc_is_set(self):
        ...
        from jaato_server.server.apparmor import resolve_plugin_apparmor_rules
        from jaato_server.shared.plugins.subagent.config import GCProfileConfig
