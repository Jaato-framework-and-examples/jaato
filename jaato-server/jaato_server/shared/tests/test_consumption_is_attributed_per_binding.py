...
REVERSIONS = [
    Reversion(
        target="jaato-server/jaato_server/shared/session_consumption.py",
        find="""        return (
...
class TestBudgetView:
    ...
    def _budgeted(self, limits, degrade=()):
        from jaato_server.shared.budget_control import BudgetControlConfig, BudgetTracker
        session = _session()
...
class TestCompletionView:
    ...
    def test_reports_the_framework_default_until_a_nudge_is_considered(self):
        from jaato_server.shared.completion_nudge import DEFAULT_MAX_COMPLETION_NUDGES
