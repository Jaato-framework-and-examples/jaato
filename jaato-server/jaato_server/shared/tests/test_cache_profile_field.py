...
from jaato_server.shared.jaato_runtime import (
    CACHE_FIELD_DELIVERY,
    cache_field_to_provider_extra,
    resolve_provider_extra,
)
from jaato_server.shared.plugins.subagent.config import CacheProfileConfig, parse_cache_block
...
REVERSIONS = [
    Reversion(
        target="jaato-server/jaato_server/shared/jaato_runtime.py",
    ...
    for entry in sorted(p.name for p in PROVIDER_DIR.iterdir() if p.is_dir()):
        ...
        try:
            mod = importlib.import_module(
                f"jaato_server.shared.plugins.model_provider.{entry}")
        except Exception:
            continue
