...
_ISHELL = "jaato-server/jaato_server/shared/plugins/interactive_shell/plugin.py"
_KERNEL_SANDBOX = "jaato-server/jaato_server/shared/plugins/notebook/kernel_sandbox.py"
...
def test_the_definition_is_pure_stdlib():
    ...
    import jaato_server.shared.apparmor_label as mod
...
def test_confine_to_profile_reports_enforce_as_confined(tmp_path, caplog):
    ...
    with caplog.at_level(logging.INFO, logger="jaato_server.server.runner.bootstrap"):
...
def test_require_enforce_refuses_a_complain_profile(tmp_path):
    ...
    from jaato_server.server.runner.bootstrap import ConfinementModeError, confine_to_profile
...
def test_a_different_profile_is_still_a_mismatch_not_a_mode_problem(tmp_path):
    ...
    from jaato_server.server.runner.bootstrap import ConfinementMismatchError, confine_to_profile
...
def test_read_current_profile_strips_the_nul_terminator(tmp_path):
    from jaato_server.server.runner.bootstrap import read_current_profile
...
def test_idempotency_skip_stays_mode_tolerant_but_announces(caplog):
    ...
    from jaato_server.server.runner.session import _maybe_self_confine
    ...
    with patch(
        "jaato_server.server.runner.bootstrap.current_confinement",
        return_value=parse_label("jaato-ws-s1 (complain)"),
    ), patch(
        "jaato_server.server.runner.bootstrap.verify_thread_confinement",
    ), patch(
        "jaato_server.server.runner.bootstrap.confine_to_profile",
    ) as confine_mock, caplog.at_level(
        logging.INFO, logger="jaato_server.server.runner.session",
    ):
...
def test_idempotency_skip_is_quiet_when_enforcing(caplog):
    ...
    with patch(
        ...
        "jaato_server.server.runner.bootstrap.confine_to_profile",
    ), caplog.at_level(logging.INFO, logger="jaato_server.server.runner.session"):
        _maybe_self_confine(envelope)
...
def test_complain_generation_warns_once_per_process(monkeypatch, caplog):
    import jaato_server.server.apparmor as apparmor_mod

    monkeypatch.setenv(COMPLAIN_ENV_VAR, "1")
    monkeypatch.setattr(apparmor_mod, "_complain_announced", __import__(
        "threading").Event())

    with caplog.at_level(logging.WARNING, logger="jaato_server.server.apparmor"):
...
def _shell_plugin(tmp_path, **config):
    from jaato_server.shared.plugins.interactive_shell.plugin import create_plugin
...
def test_require_confinement_refuses_a_complain_mode_child(tmp_path):
    ...
    try:
        with patch(
            ...
            "jaato_server.shared.plugins.interactive_shell.plugin.read_thread_label",
            create=True,
...
def test_notebook_helper_behaviour_is_unchanged(tmp_path):
    from jaato_server.shared.plugins.notebook.kernel_sandbox import apparmor_enforced_profile
    ...
    for raw, expected in cases.items():
        with patch(
            "jaato_server.shared.plugins.notebook.kernel_sandbox.try_read_label",
