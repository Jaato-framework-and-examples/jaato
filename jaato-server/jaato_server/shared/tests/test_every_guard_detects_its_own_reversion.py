...
_PACKAGES = ("jaato-server/jaato_server/shared/tests", "jaato-server/jaato_server/server/tests")
...
def _guard_modules() -> List[Tuple[str, object]]:
    ...
    for mod in pkgutil.iter_modules(dirs):
        ...
        if mod.name == Path(__file__).stem:
            continue
        try:
            pkg = ("jaato_server.shared.tests" if (ROOT / _PACKAGES[0] /
                                      f"{mod.name}.py").is_file()
                   else "jaato_server.server.tests")
...
def test_reversion_module_stays_cheap():
    ...
    src = (ROOT / "jaato-server/jaato_server/shared/tests/reversion.py").read_text()
    roots = set()
