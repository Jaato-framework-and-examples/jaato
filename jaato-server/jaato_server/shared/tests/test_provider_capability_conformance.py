...
from jaato_server.shared.plugins.model_provider.base import CAPABILITY_FIELDS  # noqa: F401
...
from jaato_server.shared.tests.test_provider_capabilities import (  # noqa: E402
    _provider_dirs,
    _read_declaration,
)
...
REVERSIONS = [
    Reversion(
        target="jaato-server/jaato_server/shared/plugins/model_provider/openrouter/converters.py",
    ...
    Reversion(
        target="jaato-server/jaato_server/shared/plugins/model_provider/_attachments.py",
        find=DISPATCH_FIXED,
...
def _load_provider_instance(provider: str):
    ...
    try:
        mod = importlib.import_module(f"jaato_server.shared.plugins.model_provider.{provider}")
