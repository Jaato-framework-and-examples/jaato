...
from jaato_server.shared.token_accounting import LEDGER_PATH_ENV, TokenLedger
from jaato_server.shared.tests.reversion import Reversion

_LEDGER = "jaato-server/jaato_server/shared/token_accounting.py"
...
def test_the_typed_key_seeds_the_env_var():
    from jaato_server.shared.plugins.subagent.config import TRACE_ENV_VARS, TraceProfileConfig
...
def test_the_typed_key_refuses_a_switch_like_its_siblings():
    import pytest
    from jaato_server.shared.plugins.subagent.config import TraceProfileConfig
    with pytest.raises(ValueError):
        TraceProfileConfig.from_dict({"ledger": "1"})


def test_the_env_catalog_records_the_typed_home():
    from jaato_server.shared.env_scope import AWAITING_TYPED_KEY, CATALOG
...
def test_an_unwritable_path_never_fails_the_record_and_is_reported_once(tmp_path, caplog):
    ...
    with caplog.at_level(logging.WARNING, logger="jaato_server.shared.token_accounting"):
        ledger._record("response", {"total_tokens": 1})
        ledger._record("response", {"total_tokens": 2})
