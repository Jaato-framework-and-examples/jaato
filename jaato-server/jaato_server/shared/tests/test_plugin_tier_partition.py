...
def _entry_point_plugin_packages() -> Dict[str, str]:
    ...
    for group in _PLUGIN_ENTRY_POINT_GROUPS:
        for _name, value in (table.get(group) or {}).items():
            ...
            parts = module.split(".")
            # Entry-point values are ``jaato_server.shared.plugins.<name>``
            # since the 1.0 namespace rename (#1079).
            if (len(parts) >= 4 and parts[0] == "jaato_server"
                    and parts[1] == "shared" and parts[2] == "plugins"):
                out[parts[3]] = group
    return out
