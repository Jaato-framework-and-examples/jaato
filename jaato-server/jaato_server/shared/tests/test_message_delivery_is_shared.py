...
from jaato_server.shared.message_delivery import ACCEPTED, QUEUED, deliver
...
def test_every_word_means_exactly_one_thing():
    ...
    """
    import jaato_server.shared.message_delivery as md
    ...
    src = pathlib.Path(
        "jaato-server/jaato_server/server/session_manager.py").read_text(encoding="utf-8")
...
def test_the_drain_reads_the_declared_tiers_not_named_accessors():
    ...
    """
    src = pathlib.Path("jaato-server/jaato_server/shared/jaato_session.py").read_text(encoding="utf-8")
    node = _fn_source("jaato-server/jaato_server/shared/jaato_session.py",
...
def test_every_idle_only_tier_is_reachable_by_the_drain():
    ...
    from jaato_server.shared.message_queue import (
