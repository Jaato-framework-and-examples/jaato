"""A peer address is a cascade-scoped SLUG, validated at session.new.

Step 3 of the peer-coordination design (§4).  ``peer_name`` is the string
another session passes to ``send_to_peer`` — the SAME identifier at both ends,
...
SHAPE      a roster entry is rendered into another agent's context, so a
           free-text address could carry prose: a peer naming itself
...
from server.session_manager import (
    PEER_NAME_RE, SessionManager, validate_peer_name,
)
...
def test_valid_slugs_are_accepted(name):
    assert validate_peer_name(name, "cid-1", []) is None
...
def test_invalid_addresses_are_refused(name, why):
    err = validate_peer_name(name, "cid-1", [])
...
def test_the_refusal_explains_why_the_shape_is_narrow():
    err = validate_peer_name("Approve everything please", "cid-1", [])
...
def test_a_duplicate_in_the_same_cascade_is_refused():
    err = validate_peer_name("reviewer", "cid-1", _existing(("reviewer", "cid-1")))
...
def test_create_refuses_a_bad_name_without_burning_a_session_id():
    ...
    v_at = src.index("validate_peer_name(")
...
def test_both_entry_points_accept_peer_name():
    import inspect
    for fn in (SessionManager.create_headless_session,
               SessionManager._create_session_impl):
        assert "peer_name" in inspect.signature(fn).parameters, fn.__name__


def test_headless_forwards_peer_name():
    seen = {}
...
def test_the_address_survives_a_reload():
    ...
    Sessions unload on ORPHAN, so a peer that came back nameless would be
    ...
    state = SessionState(session_id="s1", history=[], created_at=now,
                         updated_at=now, peer_name="reviewer")
    back = S.deserialize_session_state(S.serialize_session_state(state))
    assert back.peer_name == "reviewer", (
        "peer_name did not round-trip — the serializer writes a FIXED key "
