...
REVERSIONS = [
    Reversion(
        target="jaato-server/server/process_posture.py",
    ...
    Reversion(
        target="jaato-server/server/__main__.py",
    ...
    Reversion(
        target="jaato-server/server/websocket.py",
...
def _fresh_latch():
    ...
    """
    from server import process_posture
...
class TestTheCallSites:
    ...
    def test_the_daemon_start_applies_the_posture(self):
        from server import __main__ as daemon_main
