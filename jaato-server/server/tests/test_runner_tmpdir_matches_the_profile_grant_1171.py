...
REVERSIONS = [
    Reversion(
        target="jaato-server/server/confinement_id.py",
    ...
    Reversion(
        target="jaato-server/server/runner_spawner.py",
    ...
    Reversion(
        target="jaato-server/shared/plugins/sandbox_utils.py",
...
class TestAForkedSlotDoesNotInheritTheTemplatesTempdir:
    ...
    def test_the_pin_overrides_an_inherited_value(self):
        ...
        from server.runner.session import _pin_session_tmpdir
    ...
    def test_a_directory_that_cannot_exist_declines_to_pin(self, tmp_path):
        ...
        """
        from shared.session_envelope import SessionInitEnvelope
        from server.runner import session as runner_session
...
class TestSandboxUtilsImportsWithoutAUsableTempDir:
    ...
    def test_module_scope_resolution_is_not_fatal(self):
        from shared.plugins import sandbox_utils
