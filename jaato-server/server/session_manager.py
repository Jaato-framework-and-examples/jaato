...
class SessionManager:
    ...
    def ensure_session_loaded(
        ...
        """Programmatic trigger for the existing disk-load path.

        The non-attach counterpart to :meth:`attach_session`.  Mirrors
        the load-on-miss behaviour but skips the client-attach side
        effects (no ``attached_clients`` mutation, no
        ``_client_to_session`` write, no ``SessionRestoredEvent`` emit,
        no ``restored_pending_attach`` drain).

        Use case (server 0.6.182+): cross-session inspection / forensic
        interrogation from a daemon-resident plugin (canonical caller:
        ``jaato_premium.session_ops._resolve_target_session``).  The
        plugin is querying the session, not joining it — clients
        belong to user-driven attach paths.

        Implementation note: the existing ``_load_session`` machinery
        already does the heavy lifting (SessionPlugin.load + runner
        spawn + ``session.restore_*`` RPC dispatch + cache plugin
        lifecycle).  This method exists ONLY to expose that path
        without the attach side effects — per the audit-existing-
        machinery-before-designing-parallel-path principle.

        Args:
            session_id: The session to load.  If already loaded
                in-memory, returns it without touching disk.
            workspace_path: Caller's workspace.  Used to resolve
                ``.jaato/sessions/`` storage under that workspace.
                ``None`` falls through to the daemon's default
                storage path — works for daemons configured with a
                single global storage root, fails for per-workspace
                storage.  Most callers should pass their own
                workspace_path (assumes target session lives in the
                same workspace — the common case for cascade /
                introspector pairs).  Cross-workspace lookup is a
                Phase-2 extension (iterate known workspaces).

        Returns:
            The loaded ``Session``, or ``None`` if no on-disk state
            matches the session_id under the resolved storage path.
            Returned Session has empty ``attached_clients`` and stays
            loaded until explicit detach (no periodic sweep — see
            ``_maybe_unload_session`` call-sites; all are explicit
            triggers).

        Thread-safety: re-entrant under ``self._lock``.  The disk
        load itself runs OUTSIDE the lock (it's slow — spawns a
        runner subprocess + dispatches several RPCs over ~7-30s);
        the post-load registration into ``_sessions`` re-acquires
        under lock.  Concurrent ``get_session(session_id)`` during
        the load window returns ``None`` (the entry doesn't exist
        yet) — the caller can decide whether to retry or proceed
        with the returned Session reference from this call.
        """
        with self._lock:
        ...
        with self._lock:
            # Race-safety: another caller may have raced ahead and
            # registered the session under the same id while we were
            # loading.  Trust the existing entry (it was registered
            # first) and discard our load — disk state is identical
            # so either Session is equivalent.  The discarded loaded
            # Session will be cleaned up by GC; the runner subprocess
            # was already spawned and is now orphaned but will be
            # reclaimed by the daemon's subreaper.  This race is
            # rare (two concurrent get-misses) and the cost is one
            # extra runner spawn — acceptable tradeoff for not
            # holding _lock through the multi-second load.
            existing = self._sessions.get(session_id)
            if existing is not None:
                return existing
