...
@dataclass
class MediaDelta:
    """One chunk of binary output generated by the MODEL.

    The outbound counterpart to :class:`Attachment`: ``Attachment`` carries
    bytes a *tool* produced, ``MediaDelta`` carries bytes the *model*
    produced, arriving incrementally during a streamed completion (e.g.
    an audio-capable model speaking its answer).

    Passed to a :data:`StreamingCallback` in place of a ``str``.  A
    consumer that only handles text must ignore it -- it is emphatically
    NOT text, and coercing it with ``str()`` or concatenating it into a
    transcript corrupts both the audio and the transcript.

    Attributes:
        mime_type: MIME type of ``data`` (e.g. ``"audio/pcm"``).  Providers
            that stream headerless PCM should say so precisely, including
            rate and channels where the type allows
            (``"audio/pcm;rate=24000;channels=1"``), because a headerless
            payload carries no way to recover them.
        data: Raw binary payload for this chunk.  Not base64 -- encoding
            happens once, at the delivery boundary.
        sequence: Monotonic index within this media stream, so a consumer
