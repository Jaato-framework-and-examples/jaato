...
#: Marker every scaffolded driver carries in its module docstring.
#: Matched on the STABLE prefix: the full line names the exact verb and flags
#: used, which vary per archetype and must not be part of the test.
_SCAFFOLD_MARKER = "Generated by `jaato-scaffold"

#: Patterns that were correct once and are not any more.  Each is a shape a
#: GENERATED file can still carry from an older release, paired with what it
#: means now — a driver is not wrong for being old, it is wrong for handling
#: a contract that moved.
_STALE_PATTERNS = (
    ("if not sid",
     "checks create_session's result for falsiness — it returns `str` and "
     "RAISES now (SDK 0.16+), so that branch is dead and the failure it was "
     "written for is an exception nothing there catches"),
    ("check provider auth",
     "prints a guessed cause; auth is one of five, and the exception now "
     "states the real one"),
)

#: How many .py files to inspect before giving up.  A workspace can contain a
#: vendored tree; the doctor must stay fast enough that people keep running it.
...
def check_driver(workspace: str) -> List[Check]:
    ...
    for path in sorted(ws.rglob("*.py")):
        if scanned >= _DRIVER_SCAN_LIMIT:
            truncated = True
            break
        # Skip the places a driver never lives but a vendored copy might.
    ...
    if not found:
        detail = (
            "no scaffolded driver in this workspace — `jaato-scaffold new "
            "client --workspace .` emits the known-good recipe (connect "
            "timeout, terminal detection for BOTH a plain turn and a "
            "completion-gated one, env_file, client_type, create_session "
            "raising rather than returning None).\n"
            "  Other archetypes: fire, cascade, sweep, observer, host-tools."
        )
        if truncated:
            detail += (
                f"\n  (stopped after {_DRIVER_SCAN_LIMIT} files; a driver "
                "deeper than that was not seen)"
            )
        return [Check("driver", WARN, detail)]

    if stale:
        return [Check(
            "driver", WARN,
            "scaffolded driver(s) written against an OLDER SDK contract:\n  "
            + "\n  ".join(stale)
            + "\n  Regenerate, or port by hand — the generated code is the "
              "reference for what changed."
        )]

    return [Check("driver", PASS,
                  ", ".join(str(p.relative_to(ws)) for p in found[:3])
