...
- **EU AI Act Controls** - Regulation (EU) 2024/1689 as profile keys: a `regulatory:` declaration, the AI-disclosure announcement, `generated_by` marking, a hash-chained audit ledger with retention, an incident register with the Art. 73 clocks, memory provenance with a curation gate, and a generated Annex IV dossier; `jaato-scaffold validate` checks the declaration and escalates under `risk_class: high`

> **Running agents in Europe?** Every control above is shown doing its job in the **[EU AI Act evidence manual](docs/eu-ai-act-manual.md)** — one section per obligation, the Article, the mechanism, and a capture from a live run, regenerated by a script and never edited. The reasoning behind each mechanism, and what the framework deliberately does *not* do, is in the **[assessment](docs/design/eu-ai-act.md)**. jaato is a component supplier under Art. 25(4); the AI system is the application you build on it.
